...
public partial class dados
{
    ...
    private string bancoSuportedb = string.Empty;
...
END", bancodb, Ownerdb, usuario, senha, where);
    ...
    public void getVisual(string estilo, ref string CssFilePath, ref string CssPostfix)
    {
        ...
        /*
         * Guilherme Cruz - 04/04/2018
         * Esta versão (branch DevExpress) passou a ignorar os demais temas, e trabalhar somente com o tema MaterialCompact.
         * No futuro poderemos trabalhar com outros temas, adaptando a aplicação (CSS) para eles.
         */
        estilo = "MaterialCompact";
        CssFilePath = "~/App_Themes/" + estilo + "/{0}/styles.css";
        CssPostfix = estilo;
    }
    ...
    {
        return aplicaEstiloVisual(root, getInfoSistema("IDEstiloVisual").ToString(), isCalendar);
    }
    ...
    {
        string txtIe = @"<style type='text/css'> input[type='text'], input[type='password'], input[type='number'], select { min-height: 22px !important; }</style>";
        return new LiteralControl { Text = txtIe };
    }
    ...
    public Dictionary<string, string> getDicionarioExpressoesTraduzidas(params string[] grupos)
    {
        ...
        var type = typeof(Resources.traducao);
        var properties = type.GetProperties()
            .Where(prop => grupos.Any(grp => prop.Name.StartsWith(string.Format("{0}_", grp), StringComparison.InvariantCultureIgnoreCase)))
            .OrderBy(prop => prop.Name);
        foreach (var property in properties)
        {
            var value = property.GetValue(null) as string;
            dicionarioRetorno.Add(property.Name, value);
        }
