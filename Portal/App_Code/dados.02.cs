...
INNER JOIN  f_GetProjetosSelecaoBalanceamento({4}, -1, {2}) AS GPSB  ON (GPSB._CodigoProjeto IS NOT NULL )
    ...
    public DataSet getLinkSistemaAcessoExterno(int codigoEntidade)
    {
            comandoSQL = string.Format(
           @"SELECT Valor FROM {0}.{1}.ParametroConfiguracaoSistema WHERE Parametro in ('urlAplicacao_AcessoExterno') AND codigoEntidade = {2} ",
                bancodb, Ownerdb, codigoEntidade);
