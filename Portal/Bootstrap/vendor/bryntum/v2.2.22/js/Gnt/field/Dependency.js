    ...
    getErrors : function (value) {
        ...
        for (var i = 0; i < dependencies.length; i++) {
            ...
            if (dependencyStore.allowedDependencyTypes && !dependencyStore.isValidDependencyType(depData.type)) {
                var typeAbbrs       = this.dependencyParser.types,
            ...
            dep.setSourceId(isPredecessor ? taskId : task.getInternalId());
            dep.setTargetId(isPredecessor ? task.getInternalId() : taskId);
    ...
    applyChanges : function(task) {
        ...
        for (i = 0; i < dependencies.length; i++) {
            ...
            if (dep) {
                ...
                dep.setSourceId(isPredecessor ? predId : task.getInternalId());
                dep.setTargetId(isPredecessor ? task.getInternalId() : predId);
                dep.setType(depData.type);
                dep.setLag(depData.lag);
                dep.setLagUnit(depData.lagUnit);
