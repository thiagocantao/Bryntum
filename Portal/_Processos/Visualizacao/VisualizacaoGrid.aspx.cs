...
public partial class _Processos_Visualizacao_VisualizacaoGrid : System.Web.UI.Page
{
    #region Constants
    ...
    private void DefineConfiguracoesToolbar()
    {
        var item = ds.Lista.Single();
        var toolbar = gvDados.Toolbars.FindByName("MainToolbar");
        var toolbarItemTitulo = toolbar.Items.FindByName("titulo");
        var toolbarItemIncluir = toolbar.Items.FindByName("incluir");
        var toolbarItemLayout = toolbar.Items.FindByName("layout");
        var toolbarItemLayoutAbrir = toolbarItemLayout.Items.FindByName("abrir");
        var toolbarItemLayoutSalvar = toolbarItemLayout.Items.FindByName("salvar");
        var toolbarItemLayoutSalvarComo = toolbarItemLayout.Items.FindByName("salvar_como");
        var toolbarItemLayoutCarregarConfiguracoesOriginais = toolbarItemLayout.Items.FindByName("carregar_configuracoes_originais");
        var toolbarItemExportar = toolbar.Items.FindByName("exportar");
        var toolbarItemExportarXlsx = toolbarItemExportar.Items.FindByName("exportar_xlsx");
        var toolbarItemExportarPdf = toolbarItemExportar.Items.FindByName("exportar_pdf");
        var toolbarItemExportarRtf = toolbarItemExportar.Items.FindByName("exportar_rtf");
        var toolbarItemExportarCsv = toolbarItemExportar.Items.FindByName("exportar_csv");
        var toolbarItemPreFiltro = toolbar.Items.FindByName("preFiltro");

        //toolbarItemTitulo.Text = Request.QueryString["titulo"];
        if (item.NomeListaUsuario.ToString().Trim() == "")
            toolbarItemLayout.Text = "Layout";
        else
            toolbarItemLayout.Text = item.NomeListaUsuario;
        toolbarItemIncluir.ToolTip = Resources.traducao.incluir;
        ...
        if (lista.IndicaPreFiltroCustomizavel.Equals("N") && String.IsNullOrEmpty(lista.InstrucaoPreFiltroDados))
            toolbarItemPreFiltro.Visible = false;
        else
        {
            toolbarItemPreFiltro.Visible = true;

            if (lista.IndicaPreFiltroCustomizavel.Equals("S"))
            {
                toolbarItemPreFiltro.Enabled = true;
                toolbarItemPreFiltro.ToolTip = "";// "Filtro master";
                toolbarItemPreFiltro.Text = "Filtro master";
                toolbarItemPreFiltro.Image.Url = lista.InstrucaoPreFiltroDados.Equals("") ? "~/imagens/filtroDisponivel.png" : "~/imagens/FiltroAplicado.png";
            }
            else
            {
                toolbarItemPreFiltro.Enabled = false;
                toolbarItemPreFiltro.ToolTip = "Personalização do filtro desabilitada";
                toolbarItemPreFiltro.Text = "Filtro master";
                toolbarItemPreFiltro.Image.Url = "~/imagens/filtroDesabilitado.png";
            }
        ...
        toolbarItemExportar.ClientVisible = true;

        if (!exportaOLAPTodosFormatos)
        {
            toolbarItemExportar.Items.Clear();
            toolbarItemExportar.Image.IconID = "export_exporttoxls_16x16gray";
            toolbarItemExportar.ToolTip = Resources.traducao.VisualizacaoGrid_exportar_para_xlsx;
            toolbarItemExportar.Command = GridViewToolbarCommand.ExportToXlsx;
        }
        else
        {
            toolbarItemExportar.ToolTip = Resources.traducao.exportar;
            toolbarItemExportarXlsx.ToolTip = Resources.traducao.VisualizacaoGrid_exportar_para_xlsx;
            toolbarItemExportarPdf.ToolTip = Resources.traducao.exportar_para_pdf;
            toolbarItemExportarRtf.ToolTip = Resources.traducao.exportar_para_rtf;
        }
        ...
        toolbarItemLayoutAbrir.Text = Resources.traducao.abrir;
        toolbarItemLayoutAbrir.ToolTip = Resources.traducao.abrir_consultas_salvas;
        toolbarItemLayoutSalvar.Text = Resources.traducao.salvar;
        toolbarItemLayoutSalvar.ToolTip = Resources.traducao.salvar_consulta;
        toolbarItemLayoutSalvarComo.Text = Resources.traducao.salvar_como;
        toolbarItemLayoutSalvarComo.ToolTip = Resources.traducao.salvar_consulta_como;
        toolbarItemLayoutCarregarConfiguracoesOriginais.Text = Resources.traducao.carregar_configura__es_originais;
        toolbarItemLayoutCarregarConfiguracoesOriginais.ToolTip = Resources.traducao.carregar_configura__es_originais_da_consulta;

        toolbarItemLayout.ClientVisible = true;

        #endregion

        DefinirConfiguracoesFluxo();
    }

    private void DefinirConfiguracoesFluxo()
    {
        var itemIncluir = gvDados.Toolbars.FindByName("MainToolbar").Items.FindByName("incluir");
        if (indicaPodeIncluirNovoRegistro)
        {
            if (ds.ListaFluxo.Count > 1)
            {
                itemIncluir.Items.Clear();
                foreach (var listaFluxo in ds.ListaFluxo)
                    itemIncluir.Items.Add(listaFluxo.TituloMenu, string.Format("cf_{0}", listaFluxo.CodigoFluxo));
            }
        }
        else
        {
            itemIncluir.Visible = false;
        }
        fabAcoesGrid.Visible = !indicaRelatorio;
    }
 ...
 SELECT CodigoCampoLista,
  ...
  WHERE lc.CodigoLista = @CodigoLista");
        ...
        SetDataSourceSettings(dataSource, item.IsComandoSelectNull() ? string.Empty : item.ComandoSelect);
        ...
        gvDados.ClientSideEvents.ToolbarItemClick = String.Format(@"function(s, e){{ 
    var codigoUsuarioLogado = '{1}';
	...
	if (e.toolbarName === 'MainToolbar') {{
                ...
                {{
                    window.parent.showModalComFooter('FilterEditorPopup.aspx?cl={0}&clu={3}', 'Filtro dos dados', 300, 400, CarregarConsulta, null);
                }}
            }}
    ...
    private void ConfiguraSubLista(DsListaProcessos.ListaRow item)
    {
        ...
        SetDataSourceSettings(dataSourceDetalhe, item.IsComandoSelectNull() ? string.Empty : itemDetalhe.ComandoSelect);
 ...
 SELECT f.[CodigoFluxo],
  ...
  GROUP BY 
    ...
    private static void SetGridSettings(ASPxGridView grid, DsListaProcessos.ListaRow item)
    {
        ...
        grid.Settings.ShowGroupPanel = campos.Any(c => (c.IndicaAgrupamento ?? string.Empty).Equals("S", StringComparison.CurrentCultureIgnoreCase));
        grid.Settings.ShowFilterRow = campos.Any(c => (c.TipoFiltro ?? string.Empty).Equals("E", StringComparison.CurrentCultureIgnoreCase));
        grid.Settings.ShowFooter = campos.Any(c => !((c.TipoTotalizador ?? string.Empty).Equals("Nenhum", StringComparison.CurrentCultureIgnoreCase)));
        ...
        grid.Styles.AlternatingRow.Enabled = listaZebrada ? DefaultBoolean.True : DefaultBoolean.False;
 ...
 SELECT * FROM ListaCampo WHERE CodigoLista = @CodigoLista");
    ...
    private static void AddGridColumn(ASPxGridView grid, DsListaProcessos.ListaCampoRow campo, string nomeCampoCodigoProjeto = null)
    {
        ...
        if (campo.TipoCampo.ToUpper().Equals("BLT"))
        {
            ...
            colImg.Settings.AllowGroup = VerificaVerdadeiroOuFalso(campo.IndicaAgrupamento) ? DefaultBoolean.True : DefaultBoolean.False;
        ...
        else if (VerificaVerdadeiroOuFalso(campo.IndicaLink))
        {
            ...
            colLnk.Settings.AllowGroup = VerificaVerdadeiroOuFalso(campo.IndicaAgrupamento) ? DefaultBoolean.True : DefaultBoolean.False;
        ...
        else if (campo.TipoCampo == "DAT")
        {
            ...
            else
            {
                ...
                colTxt.Settings.AllowGroup = VerificaVerdadeiroOuFalso(campo.IndicaAgrupamento) ? DefaultBoolean.True : DefaultBoolean.False;
    ...
    private string ObtemBotaoInteragirFluxo(string etapa, int? codigoWorkflow, int? codigoInstanciaWf, int? codigoFluxo, int? codigoProjeto, int? codigoEtapaInicial, int? codigoEtapaAtual, int? ocorrenciaAtual, int? codigoStatus)
    {
        ...
        int acessoFluxo = ObtemCodigoAcesso(ref codigoWorkflow, codigoInstanciaWf, ref codigoFluxo, codigoProjeto, ref codigoEtapaInicial, codigoEtapaAtual, ocorrenciaAtual, codigoStatus);
    ...
    private int ObtemCodigoAcesso(ref int? codigoWorkflow, int? codigoInstanciaWf, ref int? codigoFluxo, int? codigoProjeto, ref int? codigoEtapaInicial, int? codigoEtapaAtual, int? ocorrenciaAtual, int? codigoStatus)
    {
        int acessoFluxo = 0;
        int retornoFluxo;
...
DECLARE @CodigoInstanciaWf bigint
 ...
 SELECT TOP 1 NomeInstancia 
  ...
  WHERE CodigoWorkflow = @CodigoWorkflow 
    ...
    private void DefineValorCamposControlados(int rowIndex)
    {
        var dataRow = gvDados.GetDataRow(rowIndex);

        codigoProjeto = GetValueFromDataRow<int>(dataRow, CONST_CodigoProjeto);
        codigoFluxo = GetValueFromDataRow<int>(dataRow, CONST_CodigoFluxo);
        codigoWorkflow = GetValueFromDataRow<int>(dataRow, CONST_CodigoWorkflow);
        codigoInstancia = GetValueFromDataRow<int>(dataRow, CONST_CodigoInstanciaWF);
        codigoEtapaInicial = GetValueFromDataRow<int>(dataRow, CONST_CodigoEtapaInicial);
        codigoEtapaAtual = GetValueFromDataRow<int>(dataRow, CONST_CodigoEtapaAtual);
        ocorrenciaAtual = GetValueFromDataRow<int>(dataRow, CONST_OcorrenciaAtual);
        codigoStatus = GetValueFromDataRow<int>(dataRow, CONST_CodigoStatus);
        if (hfGeral.Contains(CONST_Etapa))
            etapa = dataRow[(string)hfGeral[CONST_Etapa]] as string;
    }

    private T? GetValueFromDataRow<T>(DataRow dataRow, string id) where T : struct
    {
        if (hfGeral.Contains(id))
        {
            string columnName = hfGeral.Get(id) as string;
            object value = dataRow[columnName];

            if (VerificarValorNulo(value) || (value is int && ((int)value) == 0)) return null;
...
DECLARE @CodigoLista Int,
    ...
    public class DetailRowTemplate : ITemplate
    {
        ...
        public void InstantiateIn(Control container)
        {
            ...
            detailGridView.Font.Size = new FontUnit(9, UnitType.Point);
            detailGridView.BeforePerformDataSelect += detailGridView_BeforePerformDataSelect;
    ...
    {
        DsListaProcessos.ListaRow item = ds.Lista.Single(l => l.CodigoLista == codigoLista);
        e.Properties["cp_indica_possui_lista_usuario"] = (item.IndicaPossuiListaUsuario ?? string.Empty).Equals("S", StringComparison.CurrentCultureIgnoreCase);
        e.Properties["cp_navigate_url"] = (ds.ListaFluxo.Count == 1) ? GetNavigateUrl(ds.ListaFluxo.Single().CodigoFluxo) : string.Empty;
        e.Properties["cp_codigo_usuario_logado"] = codigoUsuarioLogado;
        e.Properties["cp_codigo_lista"] = codigoLista;
        e.Properties["cp_mostra_fab"] = !indicaRelatorio;
    }

    protected void gvDados_CustomDataCallback(object sender, ASPxGridViewCustomDataCallbackEventArgs e)
    {
        int rowIndex;
        if (int.TryParse(e.Parameters, out rowIndex))
            e.Result = ObtemDicionariosInformacoesFluxo(rowIndex);
    }

    private Dictionary<string, object> ObtemDicionariosInformacoesFluxo(int rowIndex)
    {
        DefineValorCamposControlados(rowIndex);
        int acessoFluxo = ObtemCodigoAcesso(ref codigoWorkflow, codigoInstancia, ref codigoFluxo, codigoProjeto, ref codigoEtapaInicial, codigoEtapaAtual, ocorrenciaAtual, codigoStatus);

        var dicionarioValores = new Dictionary<string, object>();
        dicionarioValores.Add(CONST_CodigoProjeto, codigoProjeto);
        dicionarioValores.Add(CONST_CodigoFluxo, codigoFluxo);
        dicionarioValores.Add(CONST_CodigoWorkflow, codigoWorkflow);
        dicionarioValores.Add(CONST_CodigoInstanciaWF, codigoInstancia);
        dicionarioValores.Add(CONST_CodigoEtapaInicial, codigoEtapaInicial);
        dicionarioValores.Add(CONST_CodigoEtapaAtual, codigoEtapaAtual);
        dicionarioValores.Add(CONST_OcorrenciaAtual, ocorrenciaAtual);
        dicionarioValores.Add(CONST_CodigoStatus, codigoStatus);
        dicionarioValores.Add(CONST_Etapa, ObtemValorAtualCampoControlado(CONST_Etapa));

        string actionName = null;

        if (codigoFluxo.HasValue && codigoFluxo.Value != 0 &&
            codigoInstancia.HasValue && codigoInstancia.Value != 0 &&
            codigoProjeto.HasValue && codigoProjeto.Value != 0 &&
            codigoWorkflow.HasValue && codigoWorkflow.Value != 0)
        {
            if (acessoFluxo == 0 || codigoStatus == 3 || codigoStatus == 16)
            {
                actionName = "opcoes_fluxo";
            }
            else if (acessoFluxo == 1)
            {
                //actionName = codigoFluxo.HasValue && codigoProjeto.HasValue && codigoWorkflow.HasValue && codigoInstancia.HasValue && codigoEtapaAtual.HasValue && ocorrenciaAtual.HasValue
                //    ? "opcoes_fluxo_visualizar"
                //    : "opcoes_fluxo";
                actionName = "opcoes_fluxo";
            }
            else
            {
                actionName = "opcoes_fluxo_interagir";
            }
        }

        dicionarioValores.Add("ActionName", actionName);
        return dicionarioValores;
    }

    protected void gvDados_ExportRenderBrick(object sender, ASPxGridViewExportRenderingEventArgs e)
    {
        exporter_RenderBrick(sender, e);
    }
