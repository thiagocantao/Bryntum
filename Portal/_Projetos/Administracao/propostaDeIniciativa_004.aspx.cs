...
public partial class _Projetos_Administracao_propostaDeIniciativa_004 : System.Web.UI.Page
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (int.TryParse(Request.QueryString["Altura"], out alturaInt))
                alturaTela = (alturaInt - 40).ToString();
        ...
        pageControl.TabPages[3].ClientVisible = exibirAbaCaracterizacaoProjeto;
...
SELECT CodigoModeloFormulario 
  ...
  WHERE mf.IniciaisFormularioControladoSistema LIKE 'CaracterizacaoProjeto'";
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		{
			string urlRedirecionamento = CarregaDadosFormulario();
    ...
    protected void grid_CommandButtonInitialize(object sender, DevExpress.Web.ASPxGridViewCommandButtonEventArgs e)
    {
        ...
        switch (e.ButtonType)
        {
            case ColumnCommandButtonType.Delete:
                if (grid.ClientInstanceName == "gvResultados" && !readOnly)
                {
                    podeExcluir = (grid.GetRowValues(e.VisibleIndex, "podeexcluir") + "" == "0");
                }
 ...
 VALUES
        ({2}, '{3}', {4}, '{5}', 0)"
            ...
            Response.Redirect(string.Format("propostaDeIniciativa_004.aspx?CP={2}&RO={0}&Altura={1}", (readOnly ? "S" : "N"), alturaTela, codProjeto), true);
        }
    }
    ...
    private void DefineCamposSomenteLeitura()
    {
        ...
        pnlCronograma.Enabled = permiteEdicao;
    ...
    private string CarregaDadosFormulario()
    {
		object indicaModeloTAI007 = null;
		object indicaModeloTAI008 = null;
        ...
        if (dv.Count > 0)
        {
            ...
            txtObservacoes.Value = row["Observacoes"];
            txtGerenteUnidade.Value = row["nomeGerenteUnidade"];
            indicaModeloTAI007 = row["IndicaModeloTAI007"];
			indicaModeloTAI008 = row["IndicaModeloTAI008"];
            if (!(Convert.IsDBNull(indicaModeloTAI007) || indicaModeloTAI007 == null))
                indicaModeloTAI007 = indicaModeloTAI007.ToString().ToUpper().Equals("S");
        }
		if(!(indicaModeloTAI007 == null || Convert.IsDBNull(indicaModeloTAI007)))
			return string.Format("./propostaDeIniciativa_007.aspx{0}", Request.Url.Query);
		if (!(indicaModeloTAI008 == null || Convert.IsDBNull(indicaModeloTAI008)))
			return string.Format("./propostaDeIniciativa_008.aspx{0}", Request.Url.Query);
        return string.Empty;
    }
    ...
    protected string ObtemBotaoInclusaoRegistro(string nomeGrid, string assuntoGrid)
    {
        ...
        if (nomeGrid == "gvResultados" && !temOrcamentoAtivo)
        {
            tituloBotaoDesabilitado = string.Format("Não existe orçamento ativo para poder registrar as informações de [{0}]", assuntoGrid) ;
            htmlBotaoDesabilitado = string.Format(@"<img src=""../../imagens/botoes/incluirRegDes.png"" title=""{0}"" style=""cursor: default;""/>", tituloBotaoDesabilitado);
            htmlBotaoHabilitado = string.Format(@"<img src=""../../imagens/botoes/incluirReg02.png"" alt=""Novo"" onclick=""{0}.AddNewRow();"" style=""cursor: pointer;""/>", nomeGrid);
            strRetorno = string.Format(@"<table style=""width:100%""><tr><td align=""left"">{0}</td></tr></table>"
                , temOrcamentoAtivo ? htmlBotaoHabilitado : htmlBotaoDesabilitado);
        }
        return strRetorno;
    }
...
DECLARE @CodigoEntidade INT,
    ...
    SET @CodigoEntidade = {13}
    SET @CodigoUsuarioInclusao = {14}
...
INSERT INTO {0}.{1}.TermoAbertura04
    (  [CodigoProjeto], [NomeIniciativa], [CodigoGerenteIniciativa], [NomeGerenteIniciativa], [CodigoUnidadeNegocio], [NomeUnidadeNegocio],
       [ValorEstimado], [ObjetivoGeral], [Justificativa], [Escopo], [CronogramaBasico], [Observacoes])
  VALUES
    (  @CodigoProjeto, @NomeIniciativa, @CodigoGerenteIniciativa, @NomeGerenteIniciativa, @CodigoUnidadeNegocio, @NomeUnidadeNegocio, 
       @ValorEstimado, @ObjetivoGeral, @Justificativa, @Escopo, @CronogramaBasico, @Observacoes)
...
END
 ...
 SELECT @Erro AS CodigoErro, 
    ...
    protected void gvResultados_CellEditorInitialize(object sender, ASPxGridViewEditorEventArgs e)
    {
        ...
        {
            //string tipoAgrupamento = gvResultados.GetRowValues(e.VisibleIndex, "NomeFuncaoBD").ToString();
            //string tipoCriterio = gvResultados.GetRowValues(e.VisibleIndex, "IndicaCriterio").ToString();

            ASPxSpinEdit spin = (ASPxSpinEdit)e.Editor;

            //spin.JSProperties["cp_Criterio"] = tipoCriterio;
            //spin.JSProperties["cp_Agrupamento"] = tipoAgrupamento;

            //if (tipoCriterio == "S")
            //{
            //    spin.ClientSideEvents.Validation = "function(s, e) { verificaCriterioStatus(s, e); }";
            //}
            //else
            //{
            //    switch (tipoAgrupamento)
            //    {
            //        case "SUM": spin.ClientSideEvents.Validation = "function(s, e) { verificaCriterioAcumuladoSum(s, e); }";
            //            break;
            //        case "AVG": spin.ClientSideEvents.Validation = "function(s, e) { verificaCriterioAcumuladoAvg(s, e); }";
            //            break;
            //        case "MIN": spin.ClientSideEvents.Validation = "function(s, e) { verificaCriterioAcumuladoMin(s, e); }";
            //            break;
            //        case "MAX": spin.ClientSideEvents.Validation = "function(s, e) { verificaCriterioAcumuladoMax(s, e); }";
            //            break;
            //    }
            //}
    ...
    protected void ddlResponsavel_ItemRequestedByValue(object source, ListEditItemRequestedByValueEventArgs e)
    {
        if (e.Value != null)
        {
            ...
            if (!Int64.TryParse(e.Value.ToString(), out value))
                return;
            ASPxComboBox comboBox = (ASPxComboBox)source;
            dsEquipe.SelectCommand = cDados.getSQLComboUsuariosPorID(CodigoEntidadeUsuarioResponsavel);//not in [tai04_ParceirosIniciativa]
