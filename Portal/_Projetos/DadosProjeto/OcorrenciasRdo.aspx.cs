...
public partial class _Projetos_DadosProjeto_OcorrenciasRdo : System.Web.UI.Page
{
    dados cDados;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        htmlOcorrencias.Height = new Unit(Request.QueryString["Altura"].ToString() + "px");
        string data = Request.QueryString["DataRdo"].ToString();
        ...
        if (converteu)
        {
            ...
            lblTituloTela.Text = "Ocorrências - Diário de Obra - Data: " + data;
    ...
    private void insereRDO()
    {
        string comandoSQL = string.Format(@"
                    ...
                    INSERT INTO {0}.{1}.Rdo_DadosColetados (DataRdo, CodigoProjeto)
				                                            VALUES(CONVERT(DateTime, '{2}', 103),{3});
                    SET @CodigoRDO = SCOPE_IDENTITY();", cDados.getDbName(), cDados.getDbOwner(), dtCampo, codigoProjeto);
    ...
    private void carregaDadosRDO()
    {
        ...
        if (cDados.DataSetOk(ds) && cDados.DataTableOk(ds.Tables[0]))
        {
            ...
            htmlOcorrencias.Html = dr["Ocorrencias"].ToString();
        }
        else
        {
            htmlOcorrencias.Html = "";
        }
    }
    ...
    protected void callbackSalvar_Callback(object source, DevExpress.Web.CallbackEventArgs e)
    {
        ...
        string comandoSQL = string.Format(@"UPDATE {0}.{1}.Rdo_DadosColetados SET Ocorrencias = '{2}' WHERE CodigoRdo = {3}", cDados.getDbName(), cDados.getDbOwner(), htmlOcorrencias.Html.Replace("'", "''"), codigoRDO);
        ...
        {
            callbackSalvar.JSProperties["cp_Msg"] = "Erro ao salvar a ocorrência: " + Environment.NewLine + ex.Message;
        }
    }
}