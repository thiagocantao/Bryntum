...
public partial class _Projetos_DadosProjeto_PlanejamentoSprint_antigo : System.Web.UI.Page
{
    dados cDados;
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        cDados.setaTamanhoMaximoMemo(mmObjetivos, 2000, lbl_mmObjetivos);
    ...
    private void HeaderOnTela()
    {
        ...
        Header.Controls.Add(cDados.getLiteral(@"<script type=""text/javascript"" language=""javascript"" src=""../../scripts/PlanejamentoSprint_antigo.js""></script>"));
        Header.Controls.Add(cDados.getLiteral(@"<script type=""text/javascript"" language=""javascript"" src=""../../scripts/_Strings.js""></script>"));
        Header.Controls.Add(cDados.getLiteral(@"<script type=""text/javascript"" language=""javascript"" src=""../../scripts/barraNavegacao.js""></script>"));
        this.TH(this.TS("PlanejamentoSprint", "_Strings", "barraNavegacao"));
    }
    ...
    {   // Calcula a altura da tela
        ...
        {
            mmObjetivos.Height = alturaPrincipalBackup - 735;
            mmAnotacoes.Height = alturaPrincipalBackup - 735;
        }
        else
        {
    ...
    private void salvaCampos()
    {
        ...
        if (result)
            ...
            callback.JSProperties["cp_Msg"] = "Erro ao salvar dados!";
    }
    ...
    private void carregaGridEquipe()
    {
        string comandoSQL = string.Format(@"
            BEGIN
	          ...
	          SELECT ri.CodigoRecursoIteracao
				    ...
				    ,ri.CodigoTipoPapelRecursoIteracao
						--Modificar a tabela que pega o resultado
				    ,REC.CustoHora as CustoUnitario
		    ...
		    FROM Agil_RecursoIteracao ri INNER JOIN
				 ...
				 Agil_TipoPapelRecurso tpr ON tpr.CodigoTipoPapelRecurso = ri.CodigoTipoPapelRecursoIteracao INNER JOIN
				 RecursoCorporativo as REC on  ri.CodigoRecursoCorporativo = REC.CodigoRecursoCorporativo
...
");
    ...
    protected void callbackCusto_Callback(object source, DevExpress.Web.CallbackEventArgs e)
    {
      ...
      BEGIN
        ...
        SELECT CustoHora,
               dbo.f_Agil_GetCapacidadeAlocacaoRecurso(CodigoRecursoCorporativo, 
							 (select InicioPrevisto  from Agil_Iteracao where CodigoProjetoIteracao = {1}), 
							 (select TerminoPrevisto  from Agil_Iteracao where CodigoProjetoIteracao = {1})) AS HorasDedicadas
          FROM RecursoCorporativo
         WHERE CodigoRecursoCorporativo = {0}
        
      END", ddlRecurso.Value == null ? "-1" : ddlRecurso.Value.ToString(), idProjeto);
        ...
        string comandoSQL = string.Format(@"
            SELECT ib.CodigoItem,
                   ...
                   (SELECT COUNT(1)
                     ...
                     WHERE ib_tar.CodigoItemSuperior = ib.CodigoItem and ib_tar.CodigoUsuarioExclusao is null
                       AND ib.[CodigoTipoStatusItem] <> (SELECT atsib.CodigoTipoStatusItem FROM [dbo].[Agil_TipoStatusItemBacklog] atsib WHERE atsib.IniciaisTipoStatusItemControladoSistema = 'SP_PRONTO')) AS QuantidadeTarefa
             ...
             WHERE ib.CodigoItemSuperior IS NULL
               ...
               AND ib.CodigoIteracao IN (SELECT i.CodigoIteracao FROM Agil_Iteracao AS i WHERE i.CodigoProjetoIteracao = {0})", idProjeto);
    ...
    protected void gvItens_CustomCallback(object sender, DevExpress.Web.ASPxGridViewCustomCallbackEventArgs e)
    {
        if (e.Parameters == "Editar")
        {
            ...
            , txtEsforcoPrevisto.Text.Replace(",", "."));
            ...
            if (result)
            {
                gvItens.JSProperties["cp_Msg"] = "Item salvo com sucesso!";
                carregaGridItens();
            }
            else
                gvItens.JSProperties["cp_Msg"] = "Erro ao salvar item!";
        }
        else if (e.Parameters == "Excluir")
        {
            ...
            if (result)
            {
                gvItens.JSProperties["cp_Msg"] = "Item excluído com sucesso!";
                carregaGridItens();
            }
            else
                gvItens.JSProperties["cp_Msg"] = "Erro ao excluir o item!";
        }
    ...
    private void carregaCamposPlanejamento()
    {
        ...
        if (cDados.DataSetOk(ds) && cDados.DataTableOk(ds.Tables[0]))
        {
            txtCapacidadeEquipe.Value = (decimal)ds.Tables[0].Rows[0]["Capacidade"] * (decimal)(getFatorProdutividadeEquipe());
    ...
    protected void pnCallbackGeral_Callback(object sender, DevExpress.Web.CallbackEventArgsBase e)
    {
        ...
        if (result)
        {
            pnCallbackGeral.JSProperties["cp_Msg"] = "Publicação realizada com sucesso!";
            verificaPublicacao();
            carregaGridEquipe();
            carregaGridItens();
        }
        else
            pnCallbackGeral.JSProperties["cp_Msg"] = "Erro ao publicar!";
    }
    ...
    protected void gvDados_CustomSummaryCalculate(object sender, DevExpress.Data.CustomSummaryEventArgs e)
    {
        ...
        else if (e.SummaryProcess == DevExpress.Data.CustomSummaryProcess.Finalize)
        {
            ...
            txtCapacidade_AbaEquipe.Text = string.Format("{0:n2}", HorasDedicadasEquipe * (getFatorProdutividadeEquipe()));
            txtFatorProdutividadeEquipe.Text = string.Format(@"{0:p2}", getFatorProdutividadeEquipe());
        }
    }
    ...
    protected void menu_equipe_Init(object sender, EventArgs e)
    {
        ...
        */
        ...
        cDados.setaDefinicoesBotoesInserirExportar((sender as ASPxMenu), true, "onClickBarraNavegacao('Incluir', gvDados, pcDados);", true, false, false, "PlanSpri", "Planejamento", this);
    }
