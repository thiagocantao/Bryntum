...
public partial class _Projetos_Relatorios_relAnaliseProblemasRiscos_unigest : System.Web.UI.Page
{
    dados cDados;
    ...
    protected string mensagemAusenciaDados = string.Empty;
...
- A implementação deve considerar o template em anexo.
    ...
    private void populaDDTipoProjeto()
    {
        string comandoSQL = string.Format(@"
        SELECT tp.[CodigoTipoProjeto] AS [Codigo]
        ...
        ddlTipoProjeto.DataSource = ds;
        ddlTipoProjeto.TextField = "Descricao";
        ddlTipoProjeto.ValueField = "Codigo";
...
ORDER BY 2", idEntidadeLogada);
    ...
    public void ExportReport(XtraReport report, string fileName, string fileType, bool inline)
    {
        ...
        if ((report.DataSource as DataSet).Tables["dtProjeto"].Rows.Count == 0)
        {
            mensagemAusenciaDados = "Em função dos filtros informados, não há informações a serem impressas!";
            return;
        }
    ...
    protected void btnImprimir_Click(object sender, EventArgs e)
    {
        ...
        rel_AnaliseProblemasRiscosProjetos_unigest relatorio = new rel_AnaliseProblemasRiscosProjetos_unigest(int.Parse(ddlFoco.Value == null ? "-1" : ddlFoco.Value.ToString()),
        ...
        relatorio.Parameters["pfiltro_TipoProjeto"].Value = (ddlTipoProjeto.Value == null) ? -1 : int.Parse(ddlTipoProjeto.Value.ToString());
        relatorio.Parameters["pfiltro_AfetaOrcamento"].Value = checkAfetaOOrcamento.Checked ? "S" : "N";
        relatorio.Parameters["pfiltro_AfetaMeta"].Value = checkAfetaAMeta.Checked ? "S" : "N";
