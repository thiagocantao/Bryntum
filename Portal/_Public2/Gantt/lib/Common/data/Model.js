...
export default class Model extends TreeNode(ModelStm()) {
    ...
    construct(data = {}, store = null, meta = null, skipExpose = false) {
        ...
        // Store configured with useRawData guarantees that data does not need processing (no default values will be
        // applied, no type conversion will take place)
        if (store && store.useRawData) {
            me.data = data;
        }
        else {
    ...
    set(field, value, silent = false, fromRelationUpdate = false) {
        ...
        if (me.isBatchUpdating) {
            ...
            const
                preResult = me.beforeSet ? me.beforeSet(field, value, silent, fromRelationUpdate) : undefined,
