...
export default Target => class TreeNode extends (Target || Base) {
     ...
     */
    get isLeaf() {
        return Boolean(!this.children || (this.constructor.convertEmptyParentToLeaf && !this.children.length && !this.isRoot));
    }
     ...
     * @category Parent & children
     */
    insertChild(childRecord, beforeRecord, silent = false) {
     ...
     * @category Parent & children
     */
    removeChild(childRecords, isMove = false) {
        const me = this,
            wasLeaf = me.isLeaf,
            { children, stores } = me;
    ...
    clear() {
        ...
        // only allow on root node, because we will be clearing everything
        if (!me.isRoot) return;
