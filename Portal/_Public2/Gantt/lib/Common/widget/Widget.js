...
export default class Widget extends Delayable(Events(Localizable(Base))) {
    ...
    get isVisible() {
        ...
        if (result) {
            if (!document.body.contains(element) || (this.requireSize && element.offsetWidth === 0 && element.offsetHeight === 0)) {
    ...
    doHideOrRealign(event) {
        ...
        if (event.target.nodeType === 9 && me.element.contains(activeEl) && textInputTypes[activeEl] && window.innerHeight < document.body.offsetHeight) {
            return;
        }
        ...
        else if (me.scrollAction === 'realign' && (DomHelper.isDescendant(event.target, anchoredTo) || IdHelper.fromElement(anchoredTo, isRealigningFloater)) && event.target !== (document.scrollingElement || document.body)) {
    ...
    revertFocus() {
        ...
        if (this.focusInEvent && this.focusInEvent.relatedTarget && this.focusInEvent.relatedTarget.nodeType === 1 && this.element.contains(document.activeElement)) {
