...
export default class AjaxStore extends Store {
    ...
    static get defaultConfig() {
        return {
            ...
            /**
             * Set to Â´trueÂ´ to use restful {@link #config-httpMethods}
             * @config {String}
             * @default
             * @category Remote
             */
    ...
    internalLoad(params, eventName, successFn) {
        ...
        return url ? new Promise((resolve, reject) => {
            ...
            AjaxHelper.get(event.url + me.buildQueryString(allParams), ObjectHelper.assign(options, me.fetchOptions))
                .then(async response => {
                    const
    ...
    commitAdded(commitState) {
        ...
        return added.count && me.createUrl ? new Promise((resolve) => {
            ...
            AjaxHelper.post(me.createUrl + me.buildQueryString(me.params), dataToSend, ObjectHelper.assign(options, me.fetchOptions)).then(response => {
                const
                    data    = response.parsedJson,
                    isArray = Array.isArray(data),
    ...
    commitModified(commitState) {
        const
            ...
            result       = modified.length && me.updateUrl ? new Promise(resolve => {
                ...
                const
                    modifications = modified.map(r => me.writeAllFields ? r.persistableData : r.modificationData).filter(el => !ObjectHelper.isEmpty(el));
