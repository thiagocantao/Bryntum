...
export default class Store extends Base.mixin(
        ...
        if (!me.isTree) {
            ...
            if (removedCount) {
                for (const record of removed) {
                    ...
                    else if (!record._undoingInsertion) {
    ...
    onModelChange(record, toSet, wasSet, silent, fromRelationUpdate) {
        ...
        this.storage.onItemMutation(record, wasSet);
    ...
    set id(id) {
        ...
        if (me._id) {
            Store.unregisterInstance(me);
        }
        me._id = id === true ? (id = Store.generateId('store-')) : id;
        if (id) {
            ...
            if (duplicate) {
                Store.unregisterInstance(duplicate);
            }

            Store.registerInstance(me);
        }
    }
    ...
    setStoreData(data) {
        ...
        if (data && !Array.isArray(data)) {
            data = ObjectHelper.transformNamedObjectToArray(data, me.modelClass.idField);
        }
