...
export default Target => class StoreCRUD extends (Target || Base) {
    ...
    add(records, silent = false) {
        ...
        records.forEach(data => {
            added.push(me.processRecord(data.isModel ? data : me.createRecord(data)));
        });
    ...
    insert(index, records, silent = false) {
        ...
        records.forEach(data => {
            const
                record         = me.processRecord(data.isModel ? data : me.createRecord(data)),
     ...
     * @property {Array} changes.modified Records that have changes to persistable fields
    ...
    get changes() {
        const
            me       = this,
            // Only include modified records that has persistable changes
