...
export default class EventHelper {
    ...
    static fixEvent(event) {
        ...
        if (event.target?.nodeType === 9 && 'clientX' in event) {
    ...
    static onTransitionEnd({
        ...
        property,
        handler,
        duration = DomHelper.getPropertyTransitionDuration(element, property),
        thisObj  = window
    }) {
        let timerId;
        ...
        const
            ...
            doCallback  = () => {
                ...
                if (!thisObj.isDestroyed) {
                    if (thisObj.callback) {
                        thisObj.callback(handler, thisObj);
                    }
                    else {
            ...
            detacher    = EH.on({
                element,
                transitionend({ propertyName, target }) {
        ...
        // If the transition has not signalled its end within duration + 20 milliseconds
        // then give up on it. Remove the listener and call the handler.
        if (duration != null) {
            timerId = timerSource.setTimeout(doCallback, duration + 20);
        }
...
// Set up the GlobalEvents listeners for global focus events
setupFocusListeners(document, EventHelper);
