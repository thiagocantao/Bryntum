...
export default class CalendarPanel extends Panel {
    ...
    doRefresh() {
        ...
        // The lastDomConfig property must not be preserved if we are directly manipulating the DOM
        // and other code paths in the app use DomSync.
        for (let i = 0, len = cellElements.length; i < len; i++) {
            cellElements[i].lastDomConfig = null;
        ...
        for (let columnIndex = 0; columnIndex < 7; columnIndex++) {
            ...
            if (me.headerRenderer) {
                me.headerRenderer(cell, columnIndex, cellDay);
            }
            else {
