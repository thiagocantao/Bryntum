...
export default class Menu extends Popup {
    ...
    get focusElement() {
        const
            fromParentMenu = this.parentMenu?.element.contains(DomHelper.activeElement),
    ...
    onMouseLeave(event) {
        ...
        if (!leavingToChild && shouldHideMenu) {
            ...
            if (me.element.contains(DomHelper.activeElement) && DomHelper.activeElement.matches('.b-menuitem')) {
    ...
    onInternalKeyDown(event) {
        ...
        const active = DomHelper.activeElement,
            el = this.element;
