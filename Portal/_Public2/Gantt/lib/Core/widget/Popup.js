...
export default class Popup extends Panel {
    ...
    onFocusIn(e) {
        const activeEl = DomHelper.activeElement;
        ...
        if (DomHelper.activeElement === activeEl && e.target === this.element) {
            this.focus();
        }
    }
    ...
    syncModalMask() {
        ...
        if (modal && me.isVisible) {
            ...
            const
                remainingModals = me.constructor.floatRoot.querySelectorAll('.b-modal'),
    ...
    updateClosable(closable) {
        ...
        // Ensure headerElement is shown if needed and hidden if not
        if (!this.title) {
            this.headerElement.classList[Object.values(this.tools).some(t => !t.hidden) ? 'remove' : 'add']('b-hide-display');
        }
    }
    ...
    updateTextContent(textContent) {
        ...
        this.element.classList[textContent && this.hasNoChildren ? 'add' : 'remove']('b-text-popup');

        super.updateTextContent(textContent);
    }
    ...
    get modalMask() {
        ...
        if (modalMask.nextElementSibling !== this.element) {
            Popup.floatRoot.insertBefore(modalMask, this.element);
        }
