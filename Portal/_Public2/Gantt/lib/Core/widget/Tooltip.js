...
export default class Tooltip extends Popup {
    ...
    afterConstruct() {
        ...
        if (forSelector) {
            ...
            if (!me.forElement) {
                me.trackMouse = true;
                me.forElement = Tooltip.listenersTarget;
            }
        }
            ...
            if (!me.hoverDelay || (me.forSelector && Date.now() - me.lastHidden < me.quickShowInterval)) {
                ...
                if (!me.listeningForMouseMove && !me.anchorToTarget) {
                    me.mouseMoveRemover = EventHelper.on({
                        element   : Tooltip.listenersTarget,
    ...
    changeHtml(htmlOrPromise) {
        ...
        if (ObjectHelper.isPromise(htmlOrPromise)) {
            me.showAsyncMessage();
    ...
    updateHtml(value, was) {
        ...
        if (me.hasContent) {
            ...
            if (!ObjectHelper.isPromise(value)) {
    ...
    onMouseMove(event) {
        ...
        if (!me._hidden) {
            ...
            const
                isElementOut = me.forSelector && me.activeTarget && !isHiding && target.nodeType === 1 && !target.matches(me.forSelector) && !(me.allowOver && me.element.contains(target)),
...
// In some demos this code may execute when body does not yet exist (scripttag)
if (!Tooltip.listenersTarget) {
    document.addEventListener('DOMContentLoaded', setupMouseEnterListener);
}
else {
    setupMouseEnterListener();
}

function setupMouseEnterListener() {
    EventHelper.on({
        element    : Tooltip.listenersTarget,
...
function createGlobalTooltipInstance() {
    globalInstance = new Tooltip({
        id          : 'bryntum-tooltip',
        forElement  : Tooltip.listenersTarget,
        ...
        listeners   : {
            ...
            pointerOver({ source : me, target }) {
                ...
                const config = forComponent?.tipConfig || me.gatherDataConfigs(target.dataset);
...
export function createTooltip(delay = 0) {
    if (timeoutId) {
        clearTimeout(timeoutId);
    }
    ...
    if (delay === 0) {
        ...
        timeoutId = setTimeout(createGlobalTooltipInstance, delay);
    }
}

// In some demos this code may execute when body does not yet exist (UMD)
if (!Tooltip.listenersTarget) {
    document.addEventListener('DOMContentLoaded', () => createTooltip(VersionHelper.isTestEnv ? 0 : 1000));
}
else {
    // Create the singleton after app startup (Unless in test mode when mouse will be grabbed immediately)
    createTooltip(VersionHelper.isTestEnv ? 0 : 1000);
}
