...
const
    ...
    checkNext = async(toValidate, dependencyStore, results) => {
        ...
        if (dependency) {
            ...
            results.push(
                await dependencyStore.getProject().isValidExistingDependency(dependency)
            );
...
export default class DependencyColumn extends Delayable(Column) {
    ...
    async finalizeCellEdit({ grid, record, inputField, value, oldValue, editorContext }) {
        if (record && value) {
            ...
            // Validation failed; revert the field's value
                ...
                WidgetHelper.toast(editorContext.column.L('L{Invalid dependency}'));
            }
            return valid;
        }
    }
    ...
    // * reactiveRenderer() {
    //     const
    //         incoming = this.column.field === 'predecessors',
    //         deps = incoming
    //             ? yield this.record.$.incomingDeps
    //             : yield this.record.$.outgoingDeps;
    //
    //     return yield* DependencyField.predecessorsToStringGenerator(deps, incoming ? 'from' : 'to', this.column.delimiter);
    // }
}
