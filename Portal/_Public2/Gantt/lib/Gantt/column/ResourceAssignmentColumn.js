...
export default class ResourceAssignmentColumn extends Column {
     ...
     * @param {Number} assignment The index - zero based.
     * @config {Function}
     */
    itemTpl(assignment, idx) {
    ...
    renderImage({ resource, assignment, overflowCount }) {
        const tooltip = `${assignment.toString()}${overflowCount ? ` (+${overflowCount} ${this.L('L{more resources}')})` : ''}`;

        return {
            tag       : 'img',
            className : 'b-resource-avatar',
            dataset   : {
                btipScrollAction : 'realign',
                btip             : tooltip
            },
            src : resource.image ? this.grid.resourceImageFolderPath + resource.image : this.defaultAvatar
        };
    }
    ...
    get defaultAvatar() {
        ...
        return grid.defaultResourceImageName ? grid.resourceImageFolderPath + grid.defaultResourceImageName : defaultUserImage;
    }
}
