...
export default class TaskDrag extends DragBase {
    ...
    isElementDraggable(el, event) {
        ...
        if (taskRecord.resizable && !taskRecord.isMilestone && taskResize?.resize.overAnyHandle(event, taskElement)) {
            return false;
        }
    ...
    setupProductDragData(info) {
        ...
        return { record : taskRecord, dateConstraints, eventBarEls, draggedEntities };
    }
    ...
    async updateRecords(context) {
        const
            taskRecord       = context.draggedEntities[0],
