...
export default class NewTaskRendering extends Base {
    ...
    getTaskBox(taskRecord, includeOutside = false, inner = false) {
        ...
        if (!taskRecord.startDate || !taskRecord.endDate || (taskRecord.isTask && (gantt.taskStore.isDestroyed || !gantt.taskStore.isAvailable(taskRecord)))) {
            return null;
        }
    ...
    // * getTaskBoxReactive(taskRecord, includeOutside = false, inner = false) {
    //     const
    //         me        = this,
    //         { gantt } = me;
    //
    //     if (inner) {
    //         const innerElement = me.getElementFromTaskRecord(taskRecord);
    //         if (innerElement) {
    //             return Rectangle.from(innerElement, gantt.timeAxisSubGridElement);
    //         }
    //     }
    ...
    //     if (!(yield taskRecord.$.startDate) || !(yield taskRecord.$.startDate)) {
    //         return null;
    //     }
    //
    //     const
    //         horizontalAdjustment = taskRecord.milestone ? gantt.features.dependencies.pathFinder.startArrowMargin : 0,
    //         viewRange = yield gantt.reactiveTimeAxis.$.dateRange,
    //         viewStart = viewRange.startDate,
    //         viewEnd   = viewRange.endDate,
    //         taskStart = yield taskRecord.$.startDate,
    //         // Might get here before engine has normalized
    //         taskEnd   = yield taskRecord.$.endDate,
    //         top       = gantt.taskStore.indexOf(taskRecord) * gantt.rowManager.rowOffsetHeight + gantt.barMargin,
    //         height    = gantt.rowHeight - gantt.barMargin * 2;
    //
    //     // First time, update time axis to match engine dates
    //     if (!gantt.timeAxis.hasDatesFromEngine) {
    //         gantt.timeAxis.setTimeSpan(viewStart, viewEnd);
    //         gantt.timeAxis.hasDatesFromEngine = true;
    //     }
    //
    //     let start, end = null;
    //
    //     // Clip to viewport
    //     if (includeOutside && taskStart < viewStart) {
    //         start = gantt.getCoordinateFromDate(viewStart) - horizontalAdjustment;
    //     }
    //     else if (includeOutside && taskStart > viewEnd) {
    //         start = gantt.getCoordinateFromDate(viewEnd) + horizontalAdjustment;
    //     }
    //     else {
    //         start = gantt.getCoordinateFromDate(taskStart);
    //     }
    //
    //     // Clip to viewport
    //     if (taskEnd) {
    //         if (includeOutside && taskEnd < viewStart) {
    //             end = gantt.getCoordinateFromDate(viewStart) - horizontalAdjustment;
    //         }
    //         else if (includeOutside && taskEnd > viewEnd) {
    //             end = gantt.getCoordinateFromDate(viewEnd) + horizontalAdjustment;
    //         }
    //         else {
    //             end = gantt.getCoordinateFromDate(taskEnd);
    //         }
    //     }
    //
    ...
    //     }
    //
    //     return new Rectangle(start, top, width, height);
    // }
    ...
    updateFromHorizontalScroll(scrollLeft) {
        ...
        gantt.timeView.range = { startDate, endDate };

        gantt.trigger('visibleRangeChange', { startDate, endDate });
    }
    ...
    // * reactiveRenderer() {
    //     const
    //         { client, record : taskRecord, row } = this,
    //         me = client.taskRendering,
    //         box = yield* me.getTaskBoxReactive(taskRecord);
    //
    //     if (!box) {
    //         return;
    //     }
    ...
    //                 // Used for cheaper element comparison
    //                 compareHtml : data.taskContent,
    ...
    //         //retainElement : renderData.record.instanceMeta(this.gantt).retainElement
    //         syncOptions : {
