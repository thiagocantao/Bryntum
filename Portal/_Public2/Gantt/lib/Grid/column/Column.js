...
export default class Column extends Events(Localizable(Model)) {
    ...
    static get fields() {
        return [
            ...
            /**
             * Allow filtering data in the column (if {@link Grid.feature.Filter} or {@link Grid.feature.FilterBar} feature is enabled).
             ...
             * Configuration object may be used for {@link Grid.feature.FilterBar} feature to specify `filterField`. See an example in the code
             * snippet below or check {@link Grid.feature.FilterBar} page for more details.
             *
             * ```
             * const grid = new Grid({
             *     columns : [
    ...
    remove() {
        ...
        if (focusedCell && focusedCell.columnId === this.id) {
            ...
            if (DomHelper.activeElement === grid) {
                grid.navigateRight();
            }
    ...
    get editor() {
        ...
        if (editor && !(editor instanceof Widget)) {
            ...
            if (result) {
                ...
                editor = this.data.editor = Widget.create(Object.assign(this.defaultEditor, editor, {
                    owner : this.grid
                }));
            }
        }
     ...
     *
     * @param {Number|Number[]} widthMin The minimum allowed width. If the content width
     * is less than this number, this width is used instead. If this parameter is an array,
     * the first element is `widthMin` and the seconds is `widthMax`.
     * @param {Number} widthMax The maximum allowed width. If the content width is greater
     * than this number, this width is used instead.
     */
    resizeToFitContent(widthMin, widthMax) {
        const me         = this,
            {
                grid,
                renderer,
