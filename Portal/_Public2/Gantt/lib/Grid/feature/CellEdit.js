...
export default class CellEdit extends Delayable(InstancePlugin) {
    ...
    static get defaultConfig() {
        return {
            ...
            focusCellAnimationDuration : 100
        };
    }
     ...
     * @category Internal
     */
    async getAdjacentEditableCell(cellInfo, isForward) {
        const
            { grid }           = this,
            { store, columns } = grid;
        ...
        while (rowId) {
            ...
            if (column) {
                ...
                const editRec = store.getAdjacent(cellInfo.id, isForward, false, true);
     ...
     */
    async doAddNewAtEnd(newRecordConfig) {
        const
            { grid } = this,
    ...
    async finishEditing() {
        ...
        if (editorContext) {
            ...
            this.finishEditingPromise = editorContext.editor.completeEdit(column.bindCallback(column.finalizeCellEdit));
            result = await this.finishEditingPromise;
            // reset the flag
            this.finishEditingPromise = null;
        }
    ...
    onCellClick({ source : grid, record, cellSelector, cellElement, target, event }) {
        ...
        if (column.onCellClick) {
            ...
            this.onTriggerEditEvent({ cellSelector });
        }
    }
     ...
     * @category Internal event handling
     */
    onTriggerEditEvent({ cellSelector }) {
        if (this.editorContext && !this.finishEditing()) {
            return;
        }
        this.startEditing(cellSelector);
    }
    ...
    onGridBeforeRenderRows() {
        ...
        if (this.editorContext?.editor.isVisible) {
            this.cancelEditing();
        }
    }
    ...
    async onTabKeyPress(event) {
        ...
        if (focusedCell) {
            ...
            let cellInfo = await me.getAdjacentEditableCell(focusedCell, isForward);
            ...
            if (!cellInfo && isForward && me.addNewAtEnd) {
                ...
                if (currentEditableFinalizationResult === true) {
                    await this.doAddNewAtEnd(typeof me.addNewAtEnd === 'object' ? ObjectHelper.clone(me.addNewAtEnd) : {});

                    // Re-grab the next editable cell
                    cellInfo = await me.getAdjacentEditableCell(focusedCell, isForward);
                }
            }
    ...
    onEditorFocusOut(event) {
        ...
        if (me.editorContext && !me.editor.isFinishing && me.editor.inputField.owns(event._target) &&
