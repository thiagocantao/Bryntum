...
export default class Search extends InstancePlugin {
    ...
    search(find, gotoHit = true, reapply = false) {
        ...
        if (!me.listenersInitialized) {
            grid.rowManager.on({ rendercell : me.renderCell }, me);
            store.on({ [`refresh${grid.asyncEventSuffix}`] : me.onStoreRefresh }, me);
    ...
    clearHits() {
        // clear old hits
        for (const cell of DomHelper.children(this.grid.element, '.b-search-hit')) {
