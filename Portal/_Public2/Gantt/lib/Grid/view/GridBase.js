...
export default class GridBase extends Panel.mixin(
    ...
    static get defaultConfig() {
        return {
             ...
             * });
             *
             * Please see https://bryntum.com/docs/grid/#guides/upgrades/4.0.0.md for more information.
             *
             * @deprecated 4.0.0
             * @config {Boolean}
             * @default
            ...
            /**
             * A message to be shown when a store or Crud Manager is performing a remote operation. Set to `null` to disable default load mask.
             *
             * To create a custom load mask need to subscribe to the grid's store events and show {@link Core.widget.Mask Mask}
             * on {@link Core.data.AjaxStore#event-beforeRequest beforeRequest} and hide it on {@link Core.data.AjaxStore#event-afterRequest afterRequest}.
             * Note, for Crud Manager events are different: show Mask on `beforeSend` and hide it on `requestDone` and `requestFail`.
             *
             * ```javascript
             * const grid = new Grid({
             *     loadMask : null,
             ...
             * when data is loading. The message and optional configuration from the {@link #config-loadMask} config
             * take priority over these options, just as they do for `maskDefaults`, respectively.
             *
            ...
            /**
             * A message to be shown when Crud Manager is persisting changes on the server. Set to `null` to disable default sync mask.
             * This config is similar to {@link #config-loadMask} but designed for saving data.
             *
             * To create a custom sync mask need to subscribe to the Crud Manager events and show {@link Core.widget.Mask Mask}
             * on `beforeSend` and hide it on `requestDone` and `requestFail`.
             *
             * @config {String|null}
            ...
            /**
             * Configuration values for the {@link Grid.util.ScrollManager} class.
             * @config {Object}
             * @category Scrolling
             */
            scrollManager : null,
    ...
    static getLKey() {
        return 'bf757ffb-df5b-11e9-9294-d094663d5c88';
    }
    ...
    static get deprecatedEvents() {
        return {
            cellContextMenuBeforeShow : {
                ...
                message            : '`cellContextMenuBeforeShow` event is deprecated, in favor of `cellMenuBeforeShow` event. Please see https://bryntum.com/docs/grid/#guides/upgrades/4.0.0.md for more information.'
            },
            cellContextMenuShow : {
                product            : 'Grid',
                invalidAsOfVersion : '5.0.0',
                message            : '`cellContextMenuShow` event is deprecated, in favor of `cellMenuShow` event. Please see https://bryntum.com/docs/grid/#guides/upgrades/4.0.0.md for more information.'
            },
            headerContextMenuBeforeShow : {
                product            : 'Grid',
                invalidAsOfVersion : '5.0.0',
                message            : '`headerContextMenuBeforeShow` event is deprecated, in favor of `headerMenuBeforeShow` event. Please see https://bryntum.com/docs/grid/#guides/upgrades/4.0.0.md for more information.'
            },
            headerContextMenuShow : {
                product            : 'Grid',
                invalidAsOfVersion : '5.0.0',
                message            : '`headerContextMenuShow` event is deprecated, in favor of `headerMenuShow` event. Please see https://bryntum.com/docs/grid/#guides/upgrades/4.0.0.md for more information.'
            },
            contextMenuItem : {
                product            : 'Grid',
                invalidAsOfVersion : '5.0.0',
                message            : '`contextMenuItem` event is deprecated, in favor of `cellMenuItem` and `headerMenuItem` events. Please see https://bryntum.com/docs/grid/#guides/upgrades/4.0.0.md for more information.'
            },
            contextMenuToggleItem : {
                product            : 'Grid',
                invalidAsOfVersion : '5.0.0',
                message            : '`contextMenuToggleItem` event is deprecated, in favor of `cellMenuToggleItem` and `headerMenuToggleItem` events. Please see https://bryntum.com/docs/grid/#guides/upgrades/4.0.0.md for more information.'
            }
        };
    }
    ...
    finishConfigure(config) {
        ...
        me.scrollManager = new ScrollManager(ObjectHelper.assign({
            grid            : me,
    ...
    updateEnableSticky(enableSticky) {
        this.positionMode = enableSticky ? 'position' : 'translate';
        this.element.classList[enableSticky ? 'add' : 'remove']('b-enable-sticky');
    }
        ...
        return {
            ...
            children  : [
                ...
                },
                {
                    ...
                    children  : [
                        {
                            reference : 'verticalScroller',
                            className : 'b-grid-vertical-scroller'
                        }
                    ]
                },
                {
                    reference : 'virtualScrollers',
                    className : 'b-virtual-scrollers',
                ...
                {
                    tag       : 'footer',
                    reference : 'footerContainer',
                    className : {
                        'b-grid-footer-container' : 1,
                        'b-hidden'                : this.hideFooters
                    }
                }
            ]
        };
        ...
        if (store !== oldStore) {
            ...
            if (store) {
                ...
                if (store instanceof Store) {
                    //<debug>
                    if (store.modelClass !== GridRowModel && !Object.prototype.isPrototypeOf.call(GridRowModel, store.modelClass) && !me.disableGridRowModelWarning) {
    ...
    onStoreAdd({ source : store, records, index, oldIndex, isChild, oldParent, isMove, isExpandAll }) {
        ...
        // but potentially a change in total dataset height.
            ...
            const { totalHeight } = rowManager.totalHeight;
    ...
    onStoreException({ action, type, response, exceptionType, error }) {
        ...
        if (me.masked) {
            ...
            ObjectHelper.assign(me.masked.errorDefaults, me.loadMaskError);
            me.masked.error = messageHTML;
        }
    }
    ...
    onStoreDataChange({ action, changes, source : store }) {
        ...
        if (me.isPainted && !isGroupFieldChange) {
            ...
            me.renderRows(null, action in preserveScroll);
        }
    ...
    populateCellMenu({ record, items }) {
        ...
        if (isDeprecatedFeatureUsed && me.showRemoveRowInContextMenu && !me.readOnly && record && !record.isSpecialRow) {
            const manyItems = me.selectedRecords.length > 1;

            items.removeRow = {
                text        : manyItems ? 'L{removeRows}' : 'L{removeRow}',
                localeClass : this,
                icon        : 'b-fw-icon b-icon-trash',
                name        : manyItems ? 'removeRows' : 'removeRow',
                onItem      : () => me.store.remove(manyItems ? me.selectedRecords : record)
            };
        }
    }
    ...
    // Will return any row which has some part of it in the visible viewport
    get firstVisibleRow() {
        return this.rowManager.firstVisibleRow;
    }

    // Will return the first row which is fully inside visible viewport
    get firstFullyVisibleRow() {
        return this.rowManager.firstFullyVisibleRow;
    }
...
VersionHelper.setVersion('grid', '4.0.2');
