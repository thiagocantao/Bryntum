        ...
        for (const featureName of Object.keys(features)) {
            ...
            if (config) {
                ...
                if (StringHelper.uncapitalize(featureName) !== featureName) {
                    throw new Error(`Invalid feature name '${featureName}', must start with a lowercase letter`);
                }

                const featureClass = registeredInstanceFeatures[featureName];

                if (!featureClass) {
                    throw new Error(`Feature '${featureName}' not available, make sure you have imported it`);
                }
