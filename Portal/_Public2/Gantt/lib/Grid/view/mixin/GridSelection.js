...
export default Target => class GridSelection extends (Target || Base) {
    ...
    set selectedRecords(selectedRecords) {
        const
            { selectedRecordCollection, store } = this,
            toSelect                            = [];
    ...
    /**
     * Selects all rows. If store is filterered, this will merge the selection of all visible rows with any selection made prior to filtering
     * @category Selection
     */
    selectAll() {
        const me = this;
        ...
        if (me.store.isFiltered) {
            ...
            me.selectedRecords = me.store.records.filter(r => !r.isSpecialRow);
        }
    }

    /**
     * Deselects all selected rows and cells. If store is filterered, this will unselect all visible rows only. Any selections made prior to filtering remains.
     * @category Selection
     */
    deselectAll() {
        const me = this;
        ...
        if (me.store.isFiltered) {
            me.selectedRecordCollection.remove(...me.store.records.filter(r => !r.isSpecialRow));
        }
        else {
    ...
    onStoreDataChange({ action, source : store }) {
        ...
        if (isSelectAction[action]) {
            const
                selectedRecords = this.selectedRecordCollection,
                toRemove        = [];
            ...
            selectedRecords.forEach(record => {
                ...
                if (store.includes(record)) {
                    ...
                    selectedRecords.splice(index, 1, store.getById(record.id));
                }
                else {
