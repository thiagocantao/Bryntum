...
export default Target => {
    ...
    return class AbstractCrudManagerMixin extends (Target || Base).mixin(...mixins) {
        ...
        doAutoLoad() {
            return this.load().catch(response => {
                if (!response || !this.isValidResponse(response.rawResponse, response.response)) {
                    throw response;
                }
            });
        }
        ...
        */
        ...
        scheduleAutoSync() {
            ...
            if (!me.autoSyncTimerId && !me.autoSyncSuspendCounter) {
                me.autoSyncTimerId = me.setTimeout(() => {
                    ...
                    me.sync().catch(error => {
                        if (!error || !me.isValidResponse(error.rawResponse, error.response)) {
                            throw error;
                        }
                    });
                }, me.autoSyncTimeout);
        ...
        async onLoadFailure(rawResponse, fetchOptions = {}) {
            ...
            if (!wasAborted) {
                ...
                this.emitCrudStoreEvents(request.pack.stores, 'afterRequest', { exception : true });
        ...
        processPromise(reject, resolve, context) {
            ...
            if (resolve && this.isValidResponse(rawResponse, response)) {
                ...
                const
                    message = `${StringHelper.capitalize(action)} ${cancelled ? 'canceled' : 'failed'}`,
