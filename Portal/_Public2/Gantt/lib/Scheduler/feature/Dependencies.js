...
export default class Dependencies extends InstancePlugin.mixin(
    ...
    doDisable(disable) {
        ...
        if (me.client.isPainted) {
            ...
            me.dependencyStore = me.dependencyStore || project.dependencyStore;
            me.draw();
        }
    ...
    onProjectRefresh() {
        ...
        if (!me.client.refreshSuspended) {
            ...
            else if (me.toDrawOnProjectRefresh.size) {
     ...
     * @private
     */
    onEventChange({ action, record, changes }) {
        ...
        switch (action) {
            case 'filter':
                ...
                break;
            case 'update':
                // Ignore normalization of single field
                if (Object.keys(changes).length === 1 && (changes.startDate || changes.endDate || changes.duration)) {
                    return;
                }
     ...
     * @param {Rectangle} box Box for which other boxes are requried
    ...
    draw(reLayout = false) {
        ...
        if (!me.disabled && !scheduler.suspendRendering && scheduler.store.count && scheduler.rowManager.rowCount) {
