...
export default class EventDrag extends DragBase {
    ...
    isElementDraggable(el, event) {
        ...
        return !(eventResize && eventRecord.resizable && !eventRecord.isMilestone && eventResize.resize.overAnyHandle(event, eventElement));
    }
    ...
    onDragStart({ context, event }) {
        const
            {
                eventMenu,
                eventContextMenu // TODO: 'eventContextMenu' is deprecated. Please see https://bryntum.com/docs/scheduler/#guides/upgrades/4.0.0.md for more information.
    ...
    async updateAssignments(fromScheduler, toScheduler, context, copy) {
        ...
        for (let i = 0; i < assignmentRecords.length; i++) {
            ...
            const
                ...
                newStartDate            = this.constrainDragToTimeSlot ? originalStartDate : (unifiedDrag
                    ? event1Date
        ...
        toEventStore.add(eventsToAdd)?.forEach(added => eventsToBatch.add(added));
    ...
    setupProductDragData(info) {
        const
            ...
            draggedEntities  = [assignmentRecord],
            eventBarEls      = [];
        ...
        if (scheduler.multiEventSelect && (scheduler.isAssignmentSelected(draggedEntities[0]) || me.drag.startEvent.ctrlKey)) {
            draggedEntities.push.apply(draggedEntities, me.getRelatedRecords(assignmentRecord));
        }

        // Collecting all elements to drag
        draggedEntities.forEach(assignment => {
        ...
        return { record : assignmentRecord, dateConstraints, eventBarEls, draggedEntities };
    }
    ...
    getDragData(info, event) {
        const
            dragData          = super.getDragData(info, event),
            assignmentRecords = dragData.draggedEntities,
            eventRecords      = [...new Set(assignmentRecords.map(assignment => assignment.event))];

        return ObjectHelper.assign(dragData, {
    ...
    resolveStartEndDates(proxyRect) {
        ...
        let { start : startDate, end : endDate } = scheduler.getStartEndDatesFromRectangle(proxyRect, 'round', dd.duration, !timeAxis.isContinuous);
