...
export default base => class SchedulerExporterMixin extends base {
    ...
    async prepareComponent(config) {
        ...
        await client.waitForTransition();
        ...
        if (client.currentOrientation.isHorizontalRendering) {
            me._oldScrollBuffer = client.currentOrientation.scrollBuffer;
            client.currentOrientation.scrollBuffer = 100;
        }
        ...
        if (client.features.columnLines && !client.features.columnLines.disabled) {
            const columnLinesCanvas = me.element.querySelector('.b-column-lines-canvas');
    ...
    async restoreComponent(config) {
        ...
        if (client.currentOrientation.isHorizontalRendering) {
            client.currentOrientation.scrollBuffer = this._oldScrollBuffer;
        }
