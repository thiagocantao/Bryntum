...
export default class TimelineBase extends GridBase.mixin(
    ...
    static get configurable() {
        return {
            partnerSharedConfigs : {
                value : ['viewPreset', 'timeAxis', 'timeAxisViewModel'],
    ...
    static get defaultConfig() {
        return {
             ...
             * Note that setting {@link Scheduler.view.TimelineBase#config-suppressFit} on `true`, will disable `forceFit` functionality.
             * @config {Boolean}
             * @default
             * @category Time axis
             */
             ...
             * to always fit the content to the screen size. Setting this property on `true` will disable {@link Scheduler.view.TimelineBase#config-forceFit} behaviour.
             * @config {Boolean}
             * @default false
             * @category Time axis
             */
    ...
    onPaint({ firstPaint }) {
        ...
        if (firstPaint) {
            ...
            const
                ...
                availableSpace      = this.isVertical ? this.bodyContainer.offsetHeight : timeAxisSubGrid.width;
    ...
    preserveViewCenter(fn, thisObj = this, ...args) {
        ...
        const
            { isVertical } = me,
            scroller       = isVertical ? me.scrollable : me.timeAxisSubGrid.scrollable,
        ...
        me[isVertical ? 'scrollVerticallyTo' : 'scrollHorizontallyTo'](scrollStart, false).then(() => me.scrollingToCenter = false);
    ...
    onInternalResize(element, width, height, oldWidth, oldHeight) {
        ...
        if (oldSchedulerBodyRect && (height !== oldHeight || width !== oldWidth)) {
            this.onSchedulerViewportResize(width, height, oldSchedulerBodyRect.width, oldSchedulerBodyRect.height);
        }
    }
    ...
    onTimeAxisSubGridResize({ width, height, oldWidth, oldHeight }) {
        ...
        if (me.isPainted && width !== oldWidth) {
            const schedulerBodyRect = me._bodyRectangle;

            // For the first vertical overflow (likely when rows are rendered the first time), we want to update size
            // immediately to reduce initial "jumpiness" a bit
            if (me.hasVerticalOverflow && me.$firstVerticalOverflow) {
                me.onSchedulerViewportResize.now(width, schedulerBodyRect.height, oldWidth, schedulerBodyRect.height);
                me.$firstVerticalOverflow = false;
            }
            else {
                me.onSchedulerViewportResize(width, schedulerBodyRect.height, oldWidth, schedulerBodyRect.height);
            }
        }
    }
    ...
    onSchedulerViewportResize(width, height, oldWidth, oldHeight) {
        if (this.isPainted) {
            ...
            if (me._lastOverflow == null || me._lastOverflow === subGrid.overflowingHorizontally) {
                ...
                me.setTimeout(() => {
                    me.timeAxisViewModel.availableSpace = me.isHorizontal ? subGrid.width : me._bodyRectangle.height;
                }, 0);
    ...
    get timeAxisSubGridElement() {
        // We need the TimeAxisSubGrid to exist, so regions must be inited
    ...
    updateCanvasSize() {
        const
            me    = this,
            width = me.isHorizontal ? me.timeAxisViewModel.totalSize : me.timeAxisColumn.width;
        ...
        if (me.isVertical && me.suppressFit) {
            DomHelper.setLength(me.foregroundCanvas, 'height', me.timeAxisViewModel.totalSize);
        ...
        if (width !== me.$canvasWidth) {
            ...
            me.$canvasWidth = me.timeAxisViewModel.totalSize;
    ...
    async waitForTransition() {
        if (this.isAnimating) {
            return this.await('transitionend');
        }
    }
...
VersionHelper.setVersion('scheduler', '4.0.2');
