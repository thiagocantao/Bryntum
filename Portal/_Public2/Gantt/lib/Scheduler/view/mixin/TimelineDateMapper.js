...
export default Target => class TimelineDateMapper extends (Target || Base) {
    ...
    get viewportCenterDate() {
        const
            ...
            scroller                   = isHorizontal ? me.timeAxisSubGrid.scrollable : me.scrollable,
            centerX                    = scroller.x + scroller.clientWidth / 2,
            centerY                    = scroller.y + scroller.clientHeight / 2;

        if (timeAxis.isContinuous) {
            // Calculate center pixel in the viewport.
            // Then Calculate how far through the axis range that is.
            const
                scrollCenter = isHorizontal ? (centerX / scroller.scrollWidth) : (centerY / scroller.scrollHeight),
