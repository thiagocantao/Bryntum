...
export default class HorizontalRendering extends Base.mixin(BaseRendering, AttachToProjectMixin) {
    ...
    translateToScheduleCoordinate(x) {
        ...
        const pos = x - Math.floor(this.scheduler.timeAxisSubGridElement.getBoundingClientRect().left);
    ...
    onProjectDataReady() {
        const
            me = this,
            { scheduler, toDrawOnDataReady } = me;

        if (!scheduler.refreshSuspended) {
            if (toDrawOnDataReady.size) {
                me.clearResources(toDrawOnDataReady);
                me.refreshResources(toDrawOnDataReady);
            }
    ...
    updateFromHorizontalScroll(scrollLeft) {
        const
            ...
            startDate     = scheduler.getDateFromCoordinate(Math.max(0, scrollLeft - this.scrollBuffer)),
            endDate       = returnEnd ? timeAxis.endDate : (scheduler.getDateFromCoordinate(scrollLeft + width + this.scrollBuffer) || timeAxis.endDate);
