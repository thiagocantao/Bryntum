<%@ Control Language="C#" AutoEventWireup="true" CodeFile="uc_crud_caminhoCondicional.pt-BR.ascx.cs" Inherits="administracao_UC_Workflow_uc_crud_caminhoCondicional_pt_br" %>
...
<dx:ASPxPopupControl ID="pcDesignerCondicoes" runat="server" Width="700px" ClientInstanceName="pcDesignerCondicoes" AllowDragging="True" CloseAction="CloseButton" Modal="True" PopupHorizontalAlign="WindowCenter" PopupVerticalAlign="WindowCenter" HeaderText="Expressão de caminho condicional">
	...
	incluiCampoEditorCondicao(s.GetSelectedItem().value, s.GetSelectedItem().text, 'CAMPO');
                    ...
                    <td style="width: 90px">
                        <dxcp:ASPxButton ID="btnSalvarCondicao" runat="server" Text="Salvar" Width="90px"  ClientInstanceName="btnCancelarCondicao" AutoPostBack="False">
                            <ClientSideEvents Click="function(s, e) {
                                        ...
                                        async function validaTodasAsCondicoes() {
                                          await validaExpressaoCondicional();
                                        }
                                        // Chame a função assíncrona para iniciar o processamento
                                        validaTodasAsCondicoes();
            ...
            <asp:HiddenField runat="server" Value="0" ID="hfCodigoWorkflow"></asp:HiddenField>
            <asp:HiddenField runat="server" Value="0" ID="hfCodigoFluxo"></asp:HiddenField>
        </dx:PopupControlContentControl>
    </ContentCollection>
</dx:ASPxPopupControl>
...
<dxcp:ASPxCallback ID="callbackForms" runat="server" OnCallback="callbackForms_Callback">
    ...
    var valoresInvalidos = s.cp_ValoresInvalidos.split(';');
    var valoresValidos = s.cp_ValoresValidos;
    var listaCamposCompleta = s.cp_listaCamposCompleta.split(',');
       
	for(i = 0; i &lt; valoresInvalidos.length; i++)
	{
		if(valoresInvalidos[i] != '')
	     lstFormularios.RemoveItem(lstFormularios.FindItemByValue(valoresInvalidos[i]).index);
	}
    if(valoresValidos == ''){
        window.top.mostraMensagem(traducao.uc_crud_caminhoCondicional_Nao_foi_possivel_identificar_formulario_valido_na_etapa_de_origem_do_caminho_condicional_, 'atencao', true, false, null);
    }else{
        atualizaArrayCamposCondicao(listaCamposCompleta);  
    }
}" />
</dxcp:ASPxCallback>
