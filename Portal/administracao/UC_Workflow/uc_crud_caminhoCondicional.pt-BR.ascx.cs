...
public partial class administracao_UC_Workflow_uc_crud_caminhoCondicional_pt_br : System.Web.UI.UserControl
{
    dados cDados;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (null == Request.QueryString["CF"])
            hfCodigoFluxo.Value = "0";
        else
        {
            int codigoFluxo = 0;
            string s = Request.QueryString["CF"].ToString();
            int.TryParse(s, out codigoFluxo);

            if (0 != codigoFluxo)
                hfCodigoFluxo.Value = codigoFluxo.ToString();
            else
                hfCodigoFluxo.Value = "0";
        }
    }
    ...
    protected void callbackForms_Callback(object source, DevExpress.Web.CallbackEventArgs e)
    {
        ...
        if (e.Parameter != "")
        {
            foreach (string valor in e.Parameter.Split(','))
            {
                if (valor != "")
                {
                    var arrayNomeCodigoFormulario = valor.Split('|');
                    var codigoFormulario = arrayNomeCodigoFormulario[1];
                    var nomeFormulario = arrayNomeCodigoFormulario[0];

                    if (!getFormularioValido(int.Parse(codigoFormulario)))
                    {
                        valoresInvalidos += (codigoFormulario + ";");
                    }
                    else
                    {
                        valoresValidos += (codigoFormulario + ";");
                        DataTable dtCampos = getCamposModeloFormulario(int.Parse(codigoFormulario));
                        foreach(DataRow dr in dtCampos.Rows)
                        {
                            var codigoCampo = dr["CodigoCampo"].ToString();
                            var nomeCampo = dr["NomeCampo"].ToString();

                            if(codigoCampo != "")
                            {
                                if(listaCamposCompleta == "")
                                {
                                    listaCamposCompleta = "[" + nomeFormulario + "." + nomeCampo + "];" + codigoCampo;
                                }
                                else
                                {
                                    listaCamposCompleta = listaCamposCompleta + ",[" + nomeFormulario + "." + nomeCampo + "];" + codigoCampo;
                                }
        ...
        callbackForms.JSProperties["cp_ValoresInvalidos"] = valoresInvalidos;
        callbackForms.JSProperties["cp_ValoresValidos"] = valoresValidos;
        callbackForms.JSProperties["cp_listaCamposCompleta"] = listaCamposCompleta;
    }
}