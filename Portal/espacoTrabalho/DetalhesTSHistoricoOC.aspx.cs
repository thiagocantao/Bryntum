...
public partial class espacoTrabalho_DetalhesTSHistoricoOC : System.Web.UI.Page
{
    dados cDados;
    ...
    private decimal saldoProjeto = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack && !IsCallback)
        {
            ...
            DataColumn dcCustoRestanteCalculado = new DataColumn("CustoRestanteCalculado", typeof(decimal));
            ...
            dtDadosInformados.Columns.Add(dcCustoRestanteCalculado);
            setDtDadosInformados(dtDadosInformados);

            pcDados.ActiveTabIndex = 0;
            carregaDados(idAtribucao);

            bool temApontamentos = false;
            //lblSaldoDoProjeto.Text = string.Format(@"Saldo do projeto: {0:c2}", getSaldoProjeto(out temApontamentos));
            lblSaldoDoProjeto.Text = string.Format(@"{0:c2}", getSaldoProjeto(out temApontamentos));
            hfGeral.Set("IndicaHistoricoApontamentos", temApontamentos);
            hfGeral.Set("indicaApontamentosSessao", false);
        ...
        Header.Controls.Add(cDados.getLiteral(@"<script type=""text/javascript"" language=""javascript"" src=""../scripts/espacoTrabalho_DetalhesTSHistoricoOC.js""></script>"));
  ...
  cDados.getParametrosSistema("popupTituloApontamentoOutrosCustos").Tables[0].Rows[0]["popupTituloApontamentoOutrosCustos"].ToString() : pcDados.TabPages[5].Text;
    ...
    private decimal getSaldoProjeto(out bool indicaApontamentos)
    {
        decimal saldo = new decimal();

        string comandoSQL = string.Format(@"   
       declare @codigoEntidadeContexto AS int
       declare @codigoUsuarioSistema AS int
       declare @codigoProjeto AS int
       declare @codigoAtribuicao AS int

       SET @codigoEntidadeContexto = {0}
       SET @codigoUsuarioSistema = {1}
       SET @codigoProjeto = {2}
       SET @codigoAtribuicao = {3}

       SELECT ISNULL([SaldoProjeto],0) AS SaldoProjeto, [IndicaApontamentosAtribuicoes]    
       FROM [dbo].[f_art_GetSaldoProjetoHistoricoOC] (@codigoEntidadeContexto, @codigoUsuarioSistema, @codigoProjeto, @codigoAtribuicao)", 
       codigoEntidadeLogada, codigoUsuarioLogado, codigoProjeto, idAtribucao);

        DataSet dsSaldoProjeto = cDados.getDataSet(comandoSQL);

        indicaApontamentos = false;

        if (cDados.DataSetOk(dsSaldoProjeto) && cDados.DataTableOk(dsSaldoProjeto.Tables[0]))
        {
            saldo = decimal.Parse(dsSaldoProjeto.Tables[0].Rows[0]["SaldoProjeto"].ToString());
            saldoProjeto = saldo;
            hfGeral.Set("saldoProjeto", saldo);
            indicaApontamentos = dsSaldoProjeto.Tables[0].Rows[0]["IndicaApontamentosAtribuicoes"].ToString() == "S";
        }
        return saldo;
    }
    ...
    protected void carregaGrid()
    {
        string comandoSQL = string.Format(@"
            SELECT    [CodigoAtribuicao]
             ...
             FROM [dbo].[f_art_GetOutrosCustosAtribuicaoEx] ({0}, {1},{2})", codigoEntidadeLogada, codigoUsuarioLogado, idAtribucao);
    ...
    protected void callBack_Callback(object source, DevExpress.Web.CallbackEventArgs e)
    {
        ...
        bool retorno = cDados.atualizaTarefa_PC_TS_HistoricoOC(codigoUsuarioLogado, codigoEntidadeLogada, int.Parse(idAtribucao), txtTrabalhoRestante.Text, txtTrabalhoReal.Text, inicioReal, terminoReal, mmComentariosRecurso.Text, true,   getDsGrid().Tables[0], ref msgErro);
    ...
    protected void callbackAplicar_Callback(object source, CallbackEventArgs e)
    {
        ...
        ((ASPxCallback)source).JSProperties["cpSaldoProjeto"] = "";
        ((ASPxCallback)source).JSProperties["cpTipoRecurso"] = tipoRecurso;
        ...
        try
        {
            ...
            if(tipoRecurso == "Financeiro")
            {
                ...
                dr["CustoRestanteCalculado"] = spnRestanteFinanceiro.Value == null ? decimal.Parse("0.0") : spnRestanteFinanceiro.Value;
            ...
            else
            {
                ...
                dr["CustoRestanteCalculado"] = spnValorRestante.Value == null ? decimal.Parse("0.0") : spnValorRestante.Value;
            ...
            dtLinhaSelecionadaDagrid[0]["CustoRestante"] = dr["CustoRestanteCalculado"];
    ...
    protected void gvDados_HtmlDataCellPrepared(object sender, ASPxGridViewTableDataCellEventArgs e)
    {
       ...
       // e.Cell.BorderColor = Color.Black;
        ...
        if(e.DataColumn.FieldName.Equals("CustoRestante"))
        {
            decimal valorRestante = 0;
            bool retorno = decimal.TryParse(e.GetValue("CustoRestante").ToString(), out valorRestante);
            if(retorno == true && valorRestante < 0)
            {
    ...
    protected void setDtDadosInformados(DataTable dt)
    {
        ...
        Literal lit = cDados.getLiteral(string.Format(@"<div ID='divIndicaCustoRealInformado'>{0}</div>", (dt.Rows.Count > 0)));

        System.Web.UI.Control controlex = new System.Web.UI.Control();

        controlex.Controls.Add(lit);        
        divSessao.Controls.Add(controlex);
    }
    ...
    protected void pnCallbackSaldoProjeto_Callback(object sender, CallbackEventArgsBase e)
    {
        lblSaldoDoProjeto.Text = string.Format(@"{0:c2}", decimal.Parse(e.Parameter.Replace(".",",")));
        
    }

    //protected void callbackValidaCampos_Callback(object source, CallbackEventArgs e)
    //{
    //    DataSet dstemp = getDsGrid();
    //    DataRow[] drtemp = dstemp.Tables[0].Select("[IndicaLinhaAlterada] = 'True'");
    //    ((ASPxCallback)source).JSProperties["cpIndicaApontamentosAtribuicoes"] = (drtemp.Length > 0) ? 1 : 0;
    //}
}
