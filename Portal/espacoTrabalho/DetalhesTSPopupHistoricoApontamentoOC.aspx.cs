...
public partial class espacoTrabalho_DetalhesTSPopupHistoricoApontamentoOC : System.Web.UI.Page
{
    dados cDados;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        SELECT CodigoApontamentoAtribuicao,
	       ...
	       FROM dbo.f_art_GetHistoricoApontamentoTarefa(@codigoEntidadeContexto, @codigoUsuarioSistema, @codigoAtribuicaoRecurso, {3})
	            ...
	            SELECT 
			            ...
			            , rcp.[CodigoTipoRecurso]
		            FROM 
				            [dbo].[AtribuicaoRecursoTarefa]							AS	art

				            INNER JOIN [dbo].[RecursoCronogramaProjeto]	AS [rcp]			ON
					            (			rcp.[CodigoCronogramaProjeto] = art.[CodigoCronogramaProjeto]
						            AND rcp.[CodigoRecursoProjeto] = art.[CodigoRecursoProjeto] )
		            WHERE
			            art.[CodigoAtribuicao] = @CodigoAtribuicaoRecurso;
...
", codigoEntidadeLogada, codigoUsuarioLogado, idAtribucao, ehTelaDeAprovacao ? 1 : 0);
        ...
        txtRecurso.Text = ds.Tables[1].Rows[0]["NomeRecurso"].ToString();
        txtUnidade.Text = ds.Tables[1].Rows[0]["UnidadeMedidaRecurso"].ToString();
    ...
    protected void gvDados_CustomButtonInitialize(object sender, ASPxGridViewCustomButtonEventArgs e)
    {
        if(e.VisibleIndex > -1)
        {
            ...
            {
                e.Visible = IndicaCancelarDisponivel ? DevExpress.Utils.DefaultBoolean.True : DevExpress.Utils.DefaultBoolean.False;
            }
        }
    }
