...
function onClick(s, e) {
    ...
    if (qtdSelecionados == 0) {
        window.top.mostraMensagem(traducao.PendenciasWorkflowSGDA_nenhum_registro_foi_selecionado_, 'Atencao', true, false, null);
    }else {
...
function funcaoPosModal(x) {
    if (x == 'OK') {
        window.top.mostraMensagem(traducao.PendenciasWorkflowSGDA_assinatura_realizada_com_sucesso_, 'sucesso', false, false, null);
        gvDados.PerformCallback();
    }
}