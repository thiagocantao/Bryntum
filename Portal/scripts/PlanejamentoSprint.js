...
{    
   ...
   var numAux = 0;
    ...
    if (ddlRecurso.GetValue() == null) {
        numAux++;
        mensagem += "\n" + numAux + ") " + traducao.PlanejamentoSprint_o_recurso_deve_ser_informado_;
    }
    if (ddlPapelRecurso.GetValue() == null) {
        numAux++;
        mensagem += "\n" + numAux + ") " + traducao.PlanejamentoSprint_o_papel_do_recurso_deve_ser_informado_;
    }
    if (txtPercentualDedicacao.GetText() == "")
    {
        numAux++;
        mensagem += "\n" + numAux + ") " + traducao.PlanejamentoSprint_o_percentual_de_dedica__o_deve_ser_informado_;
    }
...
function validaCamposFormularioItem() {
    ...
    if (txtEsforcoPrevisto.GetText() == "") {
        numAux++;
        mensagem += "\n" + numAux + ") " + traducao.PlanejamentoSprint_a_estimativa_deve_ser_informada_;
    }
...
function MontaCamposFormularioItem(valores) {
...
function abreSelecaoTarefas() {
    window.top.showModal('./popupAssociaItensBacklog.aspx?CP=' + gvItens.cp_CP + "&CI=" + gvItens.cpCodigoIteracao + "&CPI=" + gvItens.cpCodigoProjetoIteracao + "&ALT=490", 'Selecionar Itens', 900, 490, finalizaEdicao, null);
}
...
function abreTarefas(codigoItem, somenteLeitura) {
    window.top.showModal('./TarefasItemBacklog.aspx?CodigoItem=' + codigoItem + "&RO=" + somenteLeitura + "&IDProjeto=" + gvItens.cpCodigoProjeto + "&CPA=" + gvItens.cpCodigoProjetoAgil + '&ALT=430', 'Tarefas', 1000, 430, finalizaEdicao, null);
}
