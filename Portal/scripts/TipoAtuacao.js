...
function validaCamposFormulario() {
    ...
    if (Trim(txtTipoAtuacao.GetText()) == "") {
        numAux++;
        mensagem += "\n" + numAux + ") " + traducao.TipoAtuacao_o_tipo_de_atua__o_deve_ser_informado_;
    }
...
function OnGridFocusedRowChanged(grid, forcarMontaCampos) {
    if (forcarMontaCampos || (window.pcDados && pcDados.IsVisible())) {
        grid.GetRowValues(grid.GetFocusedRowIndex(), 'CodigoTipoAtuacao;DescricaoTipoAtuacao;', MontaCamposFormulario);
    }
}
...
function MontaCamposFormulario(values) {
    ...
    txtTipoAtuacao.SetText(DescricaoTipoAtuacao);
}
...
function desabilitaHabilitaComponentes() {
    txtTipoAtuacao.SetEnabled(window.TipoOperacao && TipoOperacao != "Consultar");
}
