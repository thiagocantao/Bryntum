...
App.Gantt = {
    ...
    init: function (serverCfg) {
        ...
        preset.displayDateFormat = dateFormat;
        ...
        var g = Ext.create('Gnt.panel.Gantt', {
            ...
            tooltipTpl: new Ext.XTemplate(
                '<ul class="taskTip">',
                    '<li><strong>' + traducao.TarefasObjetivo_tarefa + ':</strong>{Name}</li>',
                '<li><strong>' + traducao.TarefasObjetivo_in_cio + ':</strong>{[values._record.getDisplayStartDate("' + dateFormat + '")]}</li>',
                    '<li><strong>% ' + traducao.TarefasObjetivo_conclu_do + ':</strong>{PercentDone}%</li>',
                '</ul>'
            ).compile(),
            ...
            columns: [
                {
                    xtype: 'namecolumn',
                    width: 450,
                    text: traducao.TarefasObjetivo_tarefa,
                    renderer: onRender
                },
                {
                    ...
                    width: 90,
                    text: traducao.TarefasObjetivo_in_cio,
                    align: 'center',
                ...
                },
                {
                    ...
                    renderer: Ext.util.Format.dateRenderer(dateFormat),
                    width: 90,
                    text: traducao.TarefasObjetivo_t_rmino,
                    align: 'center',
                    renderer: onRenderDate
                }
            ],
...
function onRender(value, meta, record, index) {
    meta.tdCls = record.data.TaskStatusClass.trim() == "Vermelho" ? "atrasada" : "";
    return value;
}

function onRenderDate(value, meta, record, index) {
    meta.tdCls = record.data.TaskStatusClass.trim() == "Vermelho" ? "atrasada" : "";
    return value === '-' ? value : Ext.util.Format.date(value, dateFormat);// + '1'//Ext.util.Format.dateRenderer('d/m/Y'),
}
