...
function validaCamposFormulario() {
    ...
    if (txtRisco.GetText() == "") {
        ...
        mensagemErro_ValidaCamposFormulario = traducao.meusRiscos_o_campo_nome_deve_ser_informado_;
        txtRisco.Focus();
    }
    else if (ddlTipo.GetText() == "") {
        pcAbas.SetActiveTab(pcAbas.GetTab(0));
        mensagemErro_ValidaCamposFormulario = traducao.meusRiscos_o_tipo_deve_ser_informado_;
        ddlTipo.Focus();
    }
    else if (ddlProbabilidade.GetText() == "") {
        pcAbas.SetActiveTab(pcAbas.GetTab(0));
        mensagemErro_ValidaCamposFormulario = traducao.meusRiscos_a_probabilidade_deve_ser_informada_;
        ddlProbabilidade.Focus();
    }
    else if (ddlImpacto.GetText() == "") {
        pcAbas.SetActiveTab(pcAbas.GetTab(0));
        mensagemErro_ValidaCamposFormulario = traducao.meusRiscos_o_impacto_deve_ser_informado_;
        ddlImpacto.Focus();
    }
    else if (txtResponsavel.GetValue() == "") {
        pcAbas.SetActiveTab(pcAbas.GetTab(0));
        mensagemErro_ValidaCamposFormulario = traducao.meusRiscos_o_respons_vel_deve_ser_informado_;
        ...
        if (txtConsequencia.GetText().length > 2000) {
            ...
            mensagemErro_ValidaCamposFormulario = traducao.meusRiscos_a_quantidade_de_caracteres_no_campo___consequ_ncia___est__maior_que_o_m_ximo_permitido_;
            txtConsequencia.Focus();
        }
        else if (txtEstrategiaTratamento.GetText().length > 2000) {
            pcAbas.SetActiveTab(pcAbas.GetTab(1));
            mensagemErro_ValidaCamposFormulario = traducao.meusRiscos_a_quantidade_de_caracteres_no_campo___estrat_gia_para_tratamento___est__maior_que_o_m_ximo_permitido;
...
function MontaCamposFormulario(values) {
    ...
    if (values) {
        /*
            0: CodigoRiscoQuestao;          6: CodigoUsuarioResponsavel;    12: DataLimiteResolucao;
            1: DescricaoRiscoQuestao;       7: NomeUsuarioResponsavel;      13: DescricaoStatusRiscoQuestao;
            2: DetalheRiscoQuestao;         8: IncluidoEmPor;               14: DataEliminacaoCancelamento;
            3: CodigoTipoRiscoQuestao;      9: PublicaoEmPor;               15: CodigoProjeto
            4: ProbabilidadePrioridade;     10: ConsequenciaRiscoQuestao;
            5: ImpactoUrgencia;             11: TratamentoRiscoQuestao;
        */
        ...
        try {
            ...
            // atualiza a imagem referente á severidade do risco/questão
            atualizaImagemSeveridade();
            //CodigoTipoRespostaRisco;CustoRiscoQuestao;CodigoRiscoQuestaoSuperior;
            var TipoRespostaRisco = values[16] != null ? values[16] : "";
            var CustoRiscoQuestao = values[17] != null ? values[17] : "";
            var RiscoQuestaoSuperior = values[18] != null ? values[18] : "";


            spnCusto.SetValue(CustoRiscoQuestao);
            txtTipoResposta.SetText(TipoRespostaRisco);
            txtRiscoAssociado.SetText(RiscoQuestaoSuperior);
        } catch (e) { }
    }
}
...
function posSalvarComSucesso() {
    if (TipoOperacao == "Editar") {
        ...
        window.top.mostraMensagem(traducao.meusRiscos_as_informa__es_foram_salvas_com_sucesso_, 'sucesso', false, false, null);
    }
}
...
function podeMudarAba(s, e) {
    ...
    if (hfGeral.Get("CodigoRiscoQuestao") == "") {
        window.top.mostraMensagem(traducao.meusRiscos_para_ter_acesso_a_op__o___ + e.tab.GetText() + "\" " + traducao.meusRiscos___obrigat_rio_salvar_as_informa__es_da_op__o___ + pcAbas.GetTab(0).GetText() + "\"", 'Atencao', true, false, null);
        return true;
    }
...
function getValoresPlanoAcao() {
    if (window.hfGeralToDoList) {
        ...
        window.top.mostraMensagem(traducao.meusRiscos_n_o_foi_poss_vel_encontrar_o_componente_todolist, 'Atencao', true, false, null);
}
...
function desabilitaHabilitaComponentes() {
    ...
    txtEstrategiaTratamento.SetEnabled(false);
}
...
function verificaVisibilidadeBotoes() {
    ...
    if (podeEliminar) {
        ...
        obtemTextosAcao(traducao.meusRiscos_eliminar, acaoTraduzida, nomeAcao, pronomeAcao, nomeItem, pronomeItem);
        ...
        obtemTextosAcao(traducao.meusRiscos_cancelar, acaoTraduzida, nomeAcao, pronomeAcao, nomeItem, pronomeItem);
...
function onBtnAcaoCancelar_Click() {
    mostraPopupAcaoCancelar("Cancelar");
}
...
function mostraPopupAcao(acao) {
    ...
    pcComentarioAcao.SetHeaderText(nomeAcao.value + traducao.meusRiscos__de_);
    lblComentarioAcao.SetText(traducao.meusRiscos_coment__rio_sobre_a_elimina____o_do_risco_);
    mmComentarioAcao.SetText(null);
    btnAcao_pcComentarioAcao.SetText(traducao.meusRiscos_registrar_ + nomeAcao.value);
...
function mostraPopupAcaoCancelar(acao) {
    ...
    pcComentarioAcao.SetHeaderText(traducao.meusRiscos_cancelamento);
    lblComentarioAcao.SetText(traducao.meusRiscos_coment__rio_sobre_o_cancelamento_do_risco);
    mmComentarioAcao.SetText(null);
    btnAcao_pcComentarioAcao.SetText(traducao.meusRiscos_registrar_ + nomeAcao.value);
    pcComentarioAcao.Show();
}
...
function obtemTextosAcao(acao, acaoTraduzida, nomeAcao, pronomeAcao, nomeItem, pronomeItem) {
    var aba = pcAbas.GetTab(0);
    var tipoRQ;
    pronomeAcao.value = traducao.meusRiscos_a;//hfGeral.Get("artigo")
    ...
    if ("Questão" == nomeItem.value)
        pronomeItem.value = traducao.meusRiscos_a;
    else
        pronomeItem.value = traducao.meusRiscos_o;

    if (traducao.meusRiscos_eliminar == acao) {
        if ("Q" == tipoRQ) {
            acaoTraduzida.value = traducao.meusRiscos_resolver;
            nomeAcao.value = traducao.meusRiscos_resolu____o;
        }
        else
            nomeAcao.value = traducao.meusRiscos_elimina____o;
    }
    else if (traducao.meusRiscos_cancelar == acao) {
        pronomeAcao.value = traducao.meusRiscos_o;
        nomeAcao.value = traducao.meusRiscos_cancelamento;
    }
    else if (traducao.meusRiscos_excluir == acao)
        nomeAcao.value = traducao.meusRiscos_exclus__o_;
...
function onBtnAcao_pcComentarioAcao_Click() {
    ...
    if ((null == comment) || ("" == comment))
        window.top.mostraMensagem(traducao.meusRiscos_por_favor__informe_o_coment_rio_, 'Atencao', true, false, null);
    else {
...
function onPnCallBack_EndCallback_Acao(s, e) {
    ...
    if ("OK" == statusAcao) {
        ...
        if (null == msg)
            msg = traducao.meusRiscos_falha_ao_atualizar_as_informa__es_;
