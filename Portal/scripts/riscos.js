...
function validaCamposFormulario()
{
    ...
    if (txtRisco.GetText() == "")
    {
        ...
        mensagemErro_ValidaCamposFormulario += (conta++) + ") " + traducao.riscos_o_campo_ + lblRiscoQuestao.GetText().toString().replace(':', '') + traducao.riscos__deve_ser_informado_ + "\n";
        txtRisco.Focus();
    }
    if (ddlTipo.GetText() == "")
    {
        pcAbas.SetActiveTab(pcAbas.GetTab(0)); 
        mensagemErro_ValidaCamposFormulario += (conta++) + ") " + traducao.riscos_o_tipo_deve_ser_informado_ + "\n";
        ddlTipo.Focus();
    }
    if (ddlProbabilidade.GetText() == "")
    {
        pcAbas.SetActiveTab(pcAbas.GetTab(0));     
        mensagemErro_ValidaCamposFormulario += (conta++) + ") " + traducao.riscos_a_ + lblProbabilidadeUrgencia.GetText().toString().replace(':', '') + traducao.riscos__deve_ser_informada_ + "\n";
    ...
    if (ddlImpacto.GetText() == "")
    {
        ...
        if(lblImpactoPrioridade.GetText() == "Impacto:")     
            mensagemErro_ValidaCamposFormulario += (conta++) + ") " + traducao.riscos_o_ + lblImpactoPrioridade.GetText().toString().replace(':', '') + traducao.riscos__deve_ser_informado_ + "\n";
        else
            mensagemErro_ValidaCamposFormulario += (conta++) + ") " + traducao.riscos_a_ + lblImpactoPrioridade.GetText().toString().replace(':', '') + traducao.riscos__deve_ser_informada_ + "\n";
        ddlImpacto.Focus();
    }
    if (ddlResponsavel.GetText() == "")
    {
        pcAbas.SetActiveTab(pcAbas.GetTab(0));     
        mensagemErro_ValidaCamposFormulario += (conta++) + ") " + traducao.riscos_o_respons_vel_deve_ser_informado_ + "\n";
    ...
    if (window.txtConsequencia)
    {
        if (txtConsequencia.GetText().length > 2000)
        {
            ...
            mensagemErro_ValidaCamposFormulario += (conta++) + ") " + traducao.riscos_a_quantidade_de_caracteres_no_campo___consequ_ncia___est__maior_que_o_m_ximo_permitido_ + "\n";
            txtConsequencia.Focus();
        }
        else if (txtEstrategiaTratamento.GetText().length > 2000)
        {
            pcAbas.SetActiveTab(pcAbas.GetTab(1));     
            mensagemErro_ValidaCamposFormulario += (conta++) + ") " + traducao.riscos_a_quantidade_de_caracteres_no_campo___estrat_gia_para_tratamento___est__maior_que_o_m_ximo_permitido_ + "\n";
...
function MontaCamposFormulario(values) {
    ...
    GerarRelatorioEmPDF1.SetClientVisible(inclui && idiomaEPortugues);
    GerarRelatorioEmPDF2.SetClientVisible(inclui && idiomaEPortugues);
    GerarRelatorioEmPDF3.SetClientVisible(inclui && idiomaEPortugues);
    GerarRelatorioEmPDF4.SetClientVisible(inclui && idiomaEPortugues);
    GerarRelatorioEmPDF5.SetClientVisible(inclui && idiomaEPortugues);
    ...
    frmAnexosContrato = '../../espacoTrabalho/frameEspacoTrabalho_BibliotecaInterno.aspx?TA=RQ&ID=' + values[0] + '&RO=' + readOnly + '&TO=' + TipoOperacao + '&ALT=290';
    ...
    var idObj = aba.GetText()
    if ("Ris" == idObj.substr(0,3) ) {
...
function posSalvarComSucesso()
{
    if (TipoOperacao == "Editar")
    {
        ...
        GerarRelatorioEmPDF1.SetClientVisible(true);
        GerarRelatorioEmPDF2.SetClientVisible(true);
        GerarRelatorioEmPDF3.SetClientVisible(true);
        GerarRelatorioEmPDF4.SetClientVisible(true);
        GerarRelatorioEmPDF5.SetClientVisible(true);
        window.top.mostraMensagem(traducao.riscos_as_informa__es_foram_salvas_com_sucesso_, 'sucesso', false, false, null);
...
function podeMudarAba(s, e)
{
    ...
    {
        window.top.mostraMensagem(traducao.riscos_para_ter_acesso_a_op__o + " \"" + e.tab.GetText() + "\" " + traducao.riscos___obrigat_rio_salvar_as_informa__es_da_op__o + " \"" + pcAbas.GetTab(0).GetText() + "\"", traducao.riscos_atencao, true, false, null);
        return true;
    }
...
function getValoresAnexos() {
    if (atualizarURLAnexos != 'N') {
        ...
        document.getElementById('pnCallback_pcDados_pcAbas_frmAnexos').src = frmAnexosContrato;
    }
}
...
function getValoresPlanoAcao()
{
    if (window.hfGeralToDoList)
    {
       ...
       // }
        ...
        window.top.mostraMensagem(traducao.riscos_n_o_foi_poss_vel_encontrar_o_componente_todolist, 'Atencao', true, false, null);
...
function verificaVisibilidadeBotoes()
{
    ...
    if (btnEliminar.GetVisible()) {
        obtemTextosAcao(traducao.riscos_eliminar, acaoTraduzida, nomeAcao, pronomeAcao, nomeItem, pronomeItem);
    ...
    if (btnCancelar.GetVisible()) {
        obtemTextosAcao(traducao.riscos_cancelar, acaoTraduzida, nomeAcao, pronomeAcao, nomeItem, pronomeItem);
...
{
    mostraPopupAcao(traducao.riscos_eliminar);
}
...
{
    mostraPopupAcaoCancelar(traducao.riscos_cancelar);
}
...
function mostraPopupAcao(acao)
{
    ...
    pcComentarioAcao.SetHeaderText(traducao.riscos_elimina__o);
    lblComentarioAcao.SetText(traducao.riscos_coment_rio_sobre_a_elimina__o_do_risco_);
...
function mostraPopupAcaoCancelar(acao) {
    var nomeItem = new myStr();
    var pronomeItem = new myStr();
    var acaoTraduzida = new myStr();
    var nomeAcao = new myStr();
    ...
    pcComentarioAcao.SetHeaderText(traducao.riscos_cancelamento);
    lblComentarioAcao.SetText(traducao.riscos_coment_rio_sobre_o_cancelamento_do_risco);
...
function obtemTextosAcao(acao, acaoTraduzida, nomeAcao, pronomeAcao, nomeItem, pronomeItem)
{
    ...
    //Tratamento de Pronome para Masculino e Feminino em PT e em inglês utiliza-se o "The"
            ...
            pronomeAcao.value = traducao.riscos_a;
            acaoTraduzida.value = acao;

            nomeItem.value = aba.GetText();
            if ("Ris" == nomeItem.value.substr(0, 3))
                tipoRQ = "R";
            else
                tipoRQ = "Q";

            if ("Questão" == nomeItem.value)
                pronomeItem.value = traducao.riscos_a;
            else
                pronomeItem.value = traducao.riscos_o;

            if ("Elimin" == acao.substr(0, 6)) {
                if ("Q" == tipoRQ) {
                    acaoTraduzida.value = traducao.riscos_resolver;
                    nomeAcao.value = traducao.riscos_resolu__o;
                }
                else
                    nomeAcao.value = traducao.riscos_elimina__o;
            }
            else if ("Cancel" == acao.substr(0, 6)) {
                pronomeAcao.value = traducao.riscos_o;
                nomeAcao.value = traducao.riscos_cancelamento;
            }
            else if ("Exclu" == acao.substr(0, 5))
                nomeAcao.value = traducao.riscos_exclus_o;  
...
function onBtnAcao_pcComentarioAcao_Click()
{
    ...
    if ( (null == comment) || ("" == comment) )
        window.top.mostraMensagem(traducao.riscos_por_favor__informe_o_coment_rio_, 'Atencao', true, false, null);
    else
    {
...
function onPnCallBack_EndCallback_Acao(s,e)
{
    ...
    else if ("NOK" == statusAcao)
    {
        ...
        if (null == msg)
            msg = traducao.riscos_falha_ao_atualizar_as_informa__es_;
...
function onClick_btnTransforma() {
    ...
    var texto = traducao.riscos_confirma_convers_o_do_risco_em_ + lblQuestao + ' ? ';
...
function AdjustSize() {
    var height = Math.max(0, document.documentElement.clientHeight) - 50;
