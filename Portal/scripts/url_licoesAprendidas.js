...
function posSalvarComSucesso()
{
    ...
    if (TipoOperacao == "Editar")
    {
        ...
        window.top.mostraMensagem("As informações foram salvas com sucesso.", 'sucesso', false, false, null);
...
{
    var strUrl = "./RelatoriosURL/Projetos/relatorioLicoesAprendidas.aspx?";
    
    strUrl += "CLA=" + gvDados.GetRowKey(gvDados.GetFocusedRowIndex());
    window.top.showModal(strUrl, traducao.url_licoesAprendidas_li__es_aprendidas, screen.width - 60, screen.height - 250, '', null);
}

function relLicoesAprendidasMenuProjRelatorios() {
    
    var strUrl = "./RelatoriosURL/Projetos/popupRelLicoesAprendidas.aspx";
    var now = new Date();
