...
namespace CDIS
{
    public class WorkflowMobile : System.Web.UI.Page
    {
        /* ============================================================================================================================================================================
        ...
        private bool _isWebMobile = false;
        ...
        public WorkflowMobile(ClasseDados dados, int codigoUsuarioInteracao, bool isPostBack, int CodigoWorkflow, int? codigoInstanciaWorkflow, int? codigoEtapaWorkflow, int? sequenciaOcorrencia, int? codigoProjeto, Page page, string stringConexao, bool isWebMobile = false)
        {
            int CodigoEtapaAtual;
            int OcorrenciaAtual;

            _isWebMobile = isWebMobile;
            ...
            if (codigoInstanciaWorkflow != null)
            {
                ...
                string comandoSQL = string.Format(
                    @"SELECT  
		                i.[CodigoWorkflow]
	                , IsNull(CASE WHEN p.CodigoReservado IS NOT NULL THEN RTrim(LTrim(p.[CodigoReservado])) + ' - ' + p.[NomeProjeto] ELSE p.[NomeProjeto] END,CAST( ISNULL(i.[NumeroProtocolo] + ' - ' + i.[NomeInstancia],i.[NomeInstancia])  AS varchar(250) ) ) AS [NomeInstancia]                    
                    , i.[EtapaAtual]
                    , i.[NumeroProtocolo]
	                , f.[NomeFluxo]
	                , e.[NomeEtapaWf]
                ...
                FROM 
	                [EtapasInstanciasWf]									AS [ei]
                       ...
                       LEFT JOIN [dbo].[Projeto] AS p ON (p.CodigoProjeto = i.IdentificadorProjetoRelacionado)
...
", CodigoWorkflow, codigoInstanciaWorkflow.Value, codigoEtapaWorkflow.Value, sequenciaOcorrencia.Value);
                ...
                else
                {
                    ...
                    string[] dadosinst = _NomeInstaciaWorkflow.Split('-');
                    //_NomeInstaciaWorkflow = dadosinst[1] + "<br>" + dadosinst[2];
                    _CodigoEtapa = codigoEtapaWorkflow.Value;

                    //protocolo = dadosinst[0];
                    protocolo = dt.Rows[0]["NumeroProtocolo"].ToString();
        ...
        /// <returns></returns>
        public Control ObtemPainelExecucaoEtapa(string CssFilePath, string CssPostFix, string FormularioEdicao, int larguraFrame, int alturaFrame, string urlDestinoAposExecutarAcao, bool mostraBotoes)
        {
            ...
            // Mobile - Estilo - Adaptar Responsive 
            pnPrincipal.ID = "pnPrincipal";
            pnPrincipal.Attributes.Add("Style", "border: #CCCCCC 1px solid; text-align: left;margin-left:3px;margin-top:3px;background-color: #ffffff;");
        ...
        private void constroiInterfaceWorkflow_FluxoUnico(Panel pnPrincipal)
        {
            ...
            myPage.ClientScript.RegisterClientScriptBlock(GetType(), "clie",
                @"<script type='text/javascript'>
                    ...
                    function appEvent (evt) {//recebe o evento da aplicação
                        localStorage.setItem(""originMsg"", evt.origin);" + (_isWebMobile ? @"" : @"
                        if (evt.data.type === 'pinValidation'){//Verifica se o pin foi validado pela Aplicação
                            console.log(evt.data.validate);
                            if (evt.data.validate) {
                                // Pin validado - pode seguir...
                                console.log(""Pin validado - pode seguir"");
                                pnBotoes.SetVisible(false);
                                pnWorkflow.PerformCallback(codigoAcaoMobile);
                            }else{
                                //Mensagem quando o usuário cancelou a tela de pin sem validar
                                console.log(""Mensagem quando o usuário cancelou a tela de pin sem validar"");
                            }
                        }
                    ") + @"}

                    //Escuta o evento enviado pela aplicação
                    if (window.addEventListener) {
                        window.removeEventListener(""message"", appEvent, false);
                        window.addEventListener(""message"", appEvent, false);
                    }
                    else {
                        window.attachEvent(""onmessage"", appEvent);
                    }
                    ...
                    function podeAvancarFluxo(codigoWorkflow, codigoEtapa, codigoAcao, indicaCertificacaoDigital, textoAcao)
                    {
                        ...
                        {
                            mostraMensagem('Para prosseguir com o fluxo, é necessário preencher a aba de Critérios!', 'erro', false, false, null, 0, 'warning', 'pnBotoes');
                            bPodeAvancar = false;
                        }
                        
                        var msg;
                        if (codigoAcao == 1001)
                            msg = 'Confirma a tramitação do parecer?';
                        else if (codigoAcao == 1002)
                            msg = 'Confirma a finalização do preenchimento do(s) formulário(s)?';
                        else 
                            msg = mensagemCustomizada + 'Confirma a finalização desta etapa? <br>Opção selecionada: `'+textoAcao+'´';

                        if(bPodeAvancar)
                        {
                            var funcObj = { avancaFluxo: function(codigoWorkflow, codigoEtapa, codigoAcao, indicaCertificacaoDigital, textoAcao){ 
                                            AvancaFluxoOK(codigoWorkflow, codigoEtapa, codigoAcao, indicaCertificacaoDigital, textoAcao); 
                                            } }
                            codigoAcaoMobile = null;
                            mostraMensagem(msg, 'confirmacao', true, true, function(){ $('#loading').removeClass('hidden'); funcObj['avancaFluxo'](codigoWorkflow, codigoEtapa, codigoAcao, indicaCertificacaoDigital, textoAcao) }, 0, 'info', 'pnBotoes');
                        }
                    ...
                    function AvancaFluxoOK(codigoWorkflow, codigoEtapa, codigoAcao, indicaCertificacaoDigital, textoAcao)
                    {
                        ...
                        if(bPodeAvancar)
                        {
                            ...
                            if (localStorage.getItem(""originMsg"")) {
                                ...
                                console.log('Chama a tela de PIN');
                                codigoAcaoMobile = codigoAcao;
                    ...
                    function faltaCriarAlgumFormulario()
                    {
                        ...
                        for(controle = 0; controle<formularios.length;controle++)
                        {
                            ...
                            if (codigoModeloFormulario != '')
                            {
                                ...
                                {
                                    mostraMensagem('A aba ""' + FormNM + '"" precisa ser salva.', 'erro', true, false, null, 0, 'warning', 'pnBotoes');
                                    return true;
                                }
                            }
                        }
                    ...
                    {
                        if (CompareTabs(GetActiveTab(),e.tab))
                        {
                            ...
                            var destino = './" + formularioEdicao + @"CMF='+codigoModeloFormulario+codigoFormulario+readOnly+codigoInstancia+codigoEtapa+codigoOcorrenciaAtual+codigoProjeto+iniciaisFrmObr;
            ...
            <!--
            <div class=""accordion panel-group objeto-workflow"" id=""accordionExample"">
                <div class=""card"">
                    <div class=""card-header"" id=""headingOne"">
                        <h2 class=""mb-0"">
                            <button type=""button"" class=""btn btn-link"" data-toggle=""collapse"" data-target=""#collapseOne"">
                                <div class=""row titulo-workflow"">
                                    <div class=""col-xs-11"">
                                        <span>{3}</span>
                                    </div>
                                    <div class=""col-xs-1"">
                        ...
                        <div class=""card-body"">
                            <div class=""row"" >
                                <div class=""col-xs-12 col-sm-6"">
                                    <div class=""div-data"">
                                        <span class=""span-titulo"">Protocolo: </span><span>{14}</span>
                                    </div>
                                    <div class=""div-data"">
                                        <span class=""span-titulo"">Etapa Atual: </span><span>{1}</span>
                                    </div>
                                    <div class=""div-data"">
                                        <span class=""span-titulo"">Etapa Anterior: </span><span>{8}
                                    </div>
                                </div>
                                <div class=""col-xs-12 col-sm-6"">
                                    <div class=""div-data"">
                                        <span class=""span-titulo"">Solicitante: </span><span>{2}
                                    </div>
                                    <div class=""div-data"">
                                        <span class=""span-titulo"">Data: </span><span>{10}
                                    </div>
                                </div>
                                <div style=""float: right; margin-right: 10px;"">
                                    <span style=""font-family:Verdana;font-size:7pt;font-weight: bold;"" >{13}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class=""panel-group objeto-workflow"" id=""accordion"" role=""tablist"" aria-multiselectable=""true"">
            <div class=""panel panel-default"">
              ...
              <div id=""collapseOne"" class=""panel-collapse collapse in" + (_isWebMobile ? " show" : "") + @""" role=""tabpanel"" aria-labelledby=""headingOne"">
          ...
          </div>", nomeFluxo, nomeEtapa, nomeSolicitante, nomeInstancia, hintFluxo, hintEtapa, hintSolicitante, hintInstancia, nomeEtapaAnterior,
          hintEtapaAnterior, DataInicioInstancia, hintDataInicioInstancia, hintVersao, VersaoFluxo, protocolo);

            pnPrincipal.Controls.Add(getLiteral(strCabecalho));

            string strBotaoFlutuantes = string.Format(
                          @"<div id=""container-floating"">
                            <div id=""floating-button"" onclick=""window.open('" + this.urlDestinoAposExecutarAcao + @"?CM=interno' , '_self');"">
                                <i class=""fa fa-arrow-left"" ></i>
                              </div>
                            </div>");
            pnPrincipal.Controls.Add(getLiteral(strBotaoFlutuantes));
            ...
            pnWorkflow.ClientSideEvents.EndCallback =
                @"function(s, e) {
                    ...
                    { 
                        var funcObj = { eventOkAlert: function(){ 
                                            window.open('" + this.urlDestinoAposExecutarAcao + @"?CM=interno' , '_self');
                                            } 
                                        }
                        if (hfGeralWorkflow.Get('CodigoAcaoWf')=='1001')
                             mostraAlert('Parecer tramitado com sucesso!',function(){ funcObj['eventOkAlert']() }, 3500);
                        else if (hfGeralWorkflow.Get('CodigoAcaoWf')=='1002')
                             mostraAlert('Preenchimento do(s) formulário(s) finalizado com sucesso!',function(){ funcObj['eventOkAlert']() }, 3500);
                        else
                             mostraAlert('Etapa finalizada com sucesso!',function(){ funcObj['eventOkAlert']() }, 3500);
                    ...
                    {
                        mostraMensagem(hfGeralWorkflow.Get('ErroSalvar'), 'erro', true, false, null, 0, 'danger', 'pnBotoes');
            ...
            // Mobile - Se há alguma acao a ser tomada aqui
            ...
            // Mobile - Aqui comeca a montagem de cada tab e do formulario dentro dela 
            // Adaptar Responsive

            string srcPrimeiroFormulário = "";
            insereAbasFormularios(_CodigoEtapa, ref srcPrimeiroFormulário, _ModoConsulta);
            // pnPrincipal.Controls.Add(tcLinksFormuarios);

            // Mobile - Menu scrollabe - Montagem

            String active = "";
            string TabsPrimNivel = string.Format(
                               @"<header>
	                             <nav class=""cd-secondary-nav"">
		                         <ul>");

            TabsPrimNivel += TabNames + string.Format(
                               @"</ul>
	                    </nav> 
                    </header>
                       <main class=""cd-main-content sub-nav"">");
            pnPrincipal.Controls.Add(getLiteral(TabsPrimNivel));

            // Frame para a renderização de formulários // #a9a9a9
            // Adaptar Responsive
            pnPrincipal.Controls.Add(getLiteral(string.Format(
                                @"<iframe name='wfFormularios' id='wfFormularios' src='{0}' height='{1}' width='100%' scrolling='auto' frameborder='0';  ></iframe></main>", srcPrimeiroFormulário, 0)));
        ...
        private void insereAbasFormularios(int codigoEtapa, ref string srcPrimeiroFormulário, bool somenteConsulta)
        {
            string comandoSQL = string.Format(
            ...
            if (ds != null)
            {
                ...
                if (dt != null)
                {
                    ...
                    // Mobile: Para cada Aba do Frame
                    //    foreach (DataRow dr in dt.Rows)                   
                    foreach (DataRow dr in dt.Select("CodigoTipoFormulario <= 2").OrderBy(r => r["OrdemFormularioEtapa"]))
                    {
                        ...
                        // Mobile - Adiciona abas ao menu scrollable
                        TabNames += string.Format(@"<li><a class="" {1}"" data-name=""{2}"" href=""#0"">{0}</a> </li>", dr["TituloFormulario"].ToString(), active, "CMF" + dr["CodigoModeloFormulario"].ToString());
                        active = "";
                    }
                    if (sIDModeloForms != "")
        ...
        // Adaptar Responsive - Footer
        private void insereBotoesDeAcoes(int codigoEtapa, Panel pnPrincipal)
        {

            ASPxPanel botaofechar = new ASPxPanel();
            botaofechar.ClientInstanceName = "pnFechar";
            botaofechar.ClientVisible = true;
            ...
            if (mostrarBotoesAcao)
            {
                ...
                if (bInserirBotoesEncerramento || bInserirBotaoTramitarParecer || bInserirBotaoTramitarFormulario)
                {
                    ...
                    botoes.Controls.Add(
                     getLiteral(string.Format(@"<div class=""row"">")));

                    if (bInserirBotoesEncerramento)
                    {
                        botoes.Controls.Add(getLiteral(@"<div class=""col-xs-12"">"));
                        insereBotoesEncerramento(codigoEtapa, dt, botoes);
                        botoes.Controls.Add(getLiteral(@"</div>"));
                    ...
                    botoes.Controls.Add(getLiteral(
                           @"</div>"));
            ...
            if (dtBotoes != null)
            {
                foreach (DataRow dr in dtBotoes.Rows)
                {
                    ...
                    if (indicaCertificacaoDigital == "N")
                    {
                        ...
                        btnAcao.Text = dr["TextoAcao"].ToString();
                        ...
                        btnAcao.CssClass = "btn btn-default btn-acao ";
                        btnAcao.Width = new Unit("100%");
        ...
        private void insereBotaoTramitarParecer(int codigoEtapa, ASPxPanel botoes)
        {
            ...
            btnAcao.Text = "Tramitar Parecer";
            btnAcao.CssClass = "btn btn-default btn-parecer";
            btnAcao.Width = new Unit("100%");
