...
{
    [TestClass]
    public class PSescoopRelMinisterio5200ServiceUnitTest
    {
        [TestMethod]
        public void GetListPSescoopRelMinisterio5200UnitMethod()
        {
            using (var uow = new UnitOfWorkService("Data Source=10.1.0.30;Initial Catalog=DBPortalEstrategia; User ID=usrDBPortalEstrategia;Password=dbportalestrategia"))
            {
                ...
                var test = uow.GetUowService<PSescoopRelMinisterio5200Service>().GetListPSescoopRelMinisterio5200(codColigada, ano1, ano2, ano3).ToList();
